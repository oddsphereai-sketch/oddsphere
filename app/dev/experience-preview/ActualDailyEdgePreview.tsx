      ...
      <button type="button" onClick={onOpen} className="grid w-full gap-3 border-t border-white/[0.06] px-4 py-3 text-left transition hover:bg-white/[0.025] sm:grid-cols-[0.8fr_0.7fr_1.5fr] sm:px-5">
        <div>
          ...
          <div className="mt-1 flex items-baseline gap-2">
            ...
            {footballOutcome ? <span className="text-[7px] font-bold uppercase tracking-wider text-amber-200">Bet grade held</span> : <><span className="font-mono text-[10px] font-black text-gray-500">{formatAmerican(currentDisplayedPrice(market))}</span>{market.currentPriceSportsbook ? <span className="text-[7px] font-bold text-gray-600">{formatSportsbook(market.currentPriceSportsbook)}</span> : null}</>}
          </div>
        </div>
        <div>
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 line-clamp-2 text-[10px] leading-relaxed text-gray-400">{footballOutcome ? `The independent football model favors ${footballOutcome.winner}. The exact-price ${marketLabelFor(marketKey, sport)} Bet grade remains separate and Held.` : currentAwareGuidedGuide(market, game.decisionLine)}</p>
        </div>
      </button>
    </section>
  );
...
function FootballOutcomeForecast({ game }: { game: DailyEdgeGameDto }) {
  const forecast = game.footballProjection;
  if (!forecast) return null;
  const homeWins = forecast.homeWinProbability >= forecast.awayWinProbability;
  ...
  return <section className="mt-4 rounded-lg border border-violet-400/15 bg-violet-400/[0.035] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-[8px] font-black uppercase tracking-[0.15em] text-violet-200">Independent outcome forecast</p><p className="mt-1 text-lg font-black text-white">{winner} · {(probability * 100).toFixed(1)}%</p></div><span className="rounded-full border border-gray-700 px-2 py-1 text-[7px] font-black uppercase tracking-wider text-gray-400">Bet grade separate</span></div><div className="mt-3 grid grid-cols-2 gap-2 text-center"><div className={`rounded-md border px-2 py-2 ${!homeWins ? "border-violet-400/25 bg-violet-400/[0.06]" : "border-white/[0.06] bg-black/20"}`}><p className="text-[7px] font-black uppercase tracking-wider text-gray-600">{game.awayTeam}</p><p className="mt-1 font-mono text-sm font-black text-gray-100">{(forecast.awayWinProbability * 100).toFixed(1)}%</p></div><div className={`rounded-md border px-2 py-2 ${homeWins ? "border-violet-400/25 bg-violet-400/[0.06]" : "border-white/[0.06] bg-black/20"}`}><p className="text-[7px] font-black uppercase tracking-wider text-gray-600">{game.homeTeam}</p><p className="mt-1 font-mono text-sm font-black text-gray-100">{(forecast.homeWinProbability * 100).toFixed(1)}%</p></div></div><p className="mt-2 text-[8px] leading-relaxed text-gray-500">This is OddSphere&apos;s football-only winner forecast. Exact offered price, market evidence, and T-60 health determine the separate Bet grade; Held does not erase the prediction.</p></section>;
}
    ...
    >
      ...
      <div className={`flex flex-1 flex-col ${compactSoccer ? "p-3.5 sm:p-4" : "p-5 sm:p-6"}`}>
        ...
        <div className={`${compactSoccer ? "mt-3" : "mt-5"} flex flex-wrap items-baseline gap-2`}>
          ...
          {footballOutcome ? <span className="text-[9px] font-black uppercase tracking-wider text-amber-200">Bet grade held</span> : <span className="font-mono text-[12px] font-bold text-gray-500">{formatAmerican(headline.priceAmerican)}</span>}
        </div>
        ...
        <p className={`${compactSoccer ? "mt-2 text-[10px]" : "mt-3 text-[12px]"} line-clamp-2 leading-relaxed text-gray-400`}>{footballOutcome ? `Independent football model favors ${footballOutcome.winner}; exact-price ${marketLabelFor(headlineKey, sport)} Bet grade remains separate and Held.` : currentAwareGuidedGuide(headline, game.decisionLine)}</p>
