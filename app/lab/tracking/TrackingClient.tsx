...
export type TrackingPresentation = "current" | "candidate";
...
export default function LabTrackingPage({
  ...
  useEffect(() => {
    ...
    fetch("/api/lab/tracking-foundation", {
      ...
      .catch((e) => {
        ...
        const transient = e instanceof DOMException && e.name === "AbortError"
          || /timed out|522|temporarily_unavailable|failed to fetch|network/i.test(message);
        // A cold aggregate can outlive the member request while still warming
        // the shared server cache. Retry once automatically so a transient
        // cold start does not strand the member on an error page.
        if (transient && loadAttempt === 0) {
          retryTimer = window.setTimeout(() => setLoadAttempt(1), 900);
          return;
        }
  ...
  if (error !== null) {
    return <Shell presentation={presentation}><div className="mx-auto max-w-xl rounded-2xl border border-amber-300/20 bg-amber-300/[0.045] p-6 text-center"><p className="text-base font-black text-amber-100">Tracking is temporarily unavailable</p><p className="mt-2 text-sm leading-6 text-amber-100/70">{error}</p><button type="button" onClick={() => { setError(null); setLoadAttempt((attempt) => attempt + 1); }} className="mt-5 rounded-lg border border-amber-200/30 bg-amber-200/[0.09] px-4 py-2 text-xs font-black uppercase tracking-wider text-amber-50 transition hover:bg-amber-200/[0.15]">Retry tracking</button></div></Shell>;
  }
  if (data === null) {
    return <Shell presentation={presentation}><div className="text-gray-400">Loading tracking…</div></Shell>;
  }
  if (!data.tablesInitialized) {
    return (
      <Shell presentation={presentation}>
        {presentation === "current" ? <Header lastUpdated={lastUpdated} presentation={presentation} /> : null}
  ...
  return (
    <Shell presentation={presentation}>
      <Header lastUpdated={lastUpdated} presentation={presentation} />
      ...
      <Section
        ...
        collapsible={presentation === "candidate"}
      >
        {settled.length === 0 ? (
          <Card>
      ...
      {/* The live candidate uses Latest Results as the member-facing feed.
          The broader prediction activity stream remains available only in
          the current/internal presentation. */}
      {presentation === "current" ? (
        <Section eyebrow="Latest activity" title="Recent Predictions">
          {recent.length === 0 ? (
            <Card>
      ...
      {/* ── METHOD — quiet, always-visible page reference ────────────── */}
      <Section eyebrow="How records are counted" title="Method">
        <Card>
          <Glossary />
          <MethodDivider />
...
function Shell({ children, presentation = "current" }: { children: React.ReactNode; presentation?: TrackingPresentation }) {
  return (
    <div
    ...
    >
      <div className={`${presentation === "candidate" ? "max-w-6xl" : "max-w-4xl"} mx-auto`}>{children}</div>
    </div>
  );
}

function Header({ lastUpdated, presentation = "current" }: { lastUpdated: string | null; presentation?: TrackingPresentation }) {
  if (presentation === "candidate") {
    return (
      <header className="mb-8 border-b border-white/[0.07] pb-5 sm:mb-10 sm:pb-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-violet-300/80">OddSphere</p>
            <h1 className="mt-1 text-[28px] font-black tracking-[-0.025em] text-white sm:text-[34px]">Tracking</h1>
            <p className="mt-1 text-[12px] font-medium text-gray-400 sm:text-[13px]">Results by sport, model, and prediction category.</p>
          </div>
          <div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.13em] text-gray-500">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" aria-hidden="true" />
            {lastUpdated !== null
              ? `Updated ${new Date(lastUpdated).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit" })}`
              : "Updates after grading"}
          </div>
        </div>
      </header>
    );
...
function Section({ eyebrow, title, children, collapsible = false }: { eyebrow?: string; title: string; children: React.ReactNode; collapsible?: boolean }) {
  if (collapsible) {
    return (
      <details className="group mb-4 overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.018] sm:mb-5">
        <summary className="flex cursor-pointer list-none items-center gap-3 px-4 py-4 sm:px-5">
          <h2 className="text-[16px] font-bold text-gray-50">{title}</h2>
          {eyebrow !== undefined ? <span className="ml-auto text-[9px] font-bold uppercase tracking-[0.17em] text-gray-500">{eyebrow}</span> : null}
          <span className="text-gray-600 transition-transform group-open:rotate-180">⌄</span>
        </summary>
        <div className="border-t border-white/[0.05] px-4 py-5 sm:px-5">{children}</div>
      </details>
    );
  }
  return (
    ...
    <>
      <CategoryTrackingBoard
        rows={records.map((record) => ({
          ...
          detail: lifetimeSourceLabel(record),
            ...
            <CategoryBars
              rows={group.rows.map((row) => ({
                ...
                sublabel: row.detail !== undefined
                  ? `${shortMarket(row.market)} · ${row.detail}`
                  : shortMarket(row.market),
                metrics: row.metrics,
              }))}
          ...
          <section key={group.sport} className="py-5 first:pt-0 last:pb-0">
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              {group.rows.map((b) => {
                ...
                const le = b.leans;
                return (
                  <article key={`${b.sport}-${b.market}`} className="rounded-xl border border-white/[0.05] bg-white/[0.012] p-3.5">
                    ...
                    <div className="grid gap-2 sm:grid-cols-2">
                      <BestAnglesPanel label="Best Angle" color="rgb(52 211 153)" metrics={ba} decided={ba.wins + ba.losses} />
                      <BestAnglesPanel label="Lean" color="rgb(129 140 248)" metrics={le} decided={le.wins + le.losses} />
                    </div>
                  </article>
                );
              })}
...
function AllActionableFooter({ overall, bestAngles, leans }: { overall: Metrics; bestAngles: Metrics; leans?: Metrics }) {
  ...
  return (
    <div>
      <div className="text-[10px] uppercase tracking-[0.16em] font-bold text-gray-500 mb-2">All tracked predictions</div>
