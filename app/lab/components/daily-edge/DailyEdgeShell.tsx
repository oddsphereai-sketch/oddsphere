...
function EdgeStackClean({ market, marketData }: { market: MarketKey; marketData: MarketEdgeDto }) {
  ...
  const oddsTrailCurrent =
    displayPriceAmerican(marketData) ??
    marketData.marketReadV2?.movement?.currentPrice ??
    fiBoardTrail?.current ??
    null;
  const hasLiveCurrentAfterLock =
    marketData.lockedLineAmerican != null &&
    marketData.bestAvailablePriceAmerican != null &&
    marketData.bestAvailablePriceAmerican !== marketData.lockedLineAmerican;
  const persistedOddsTrail: NonNullable<MarketEdgeDto["oddsTrail"]> = hasLiveCurrentAfterLock
    ? [
        ...(marketData.oddsTrail ?? []),
        {
          american: marketData.bestAvailablePriceAmerican!,
          line: marketData.line ?? marketData.marketTotal ?? null,
          observedAt: marketData.bestAvailableObservedAt ?? null,
          sportsbook: marketData.bestAvailableSportsbook ?? null,
          source: "current_line",
          label: "current",
        },
      ]
        ...
        <div className="border-t border-white/[0.04] py-2.5">
          ...
          {persistedOddsTrail.length > 0 ? (
            <CleanPersistedOddsTrail
              ...
              hasLiveCurrentAfterLock={hasLiveCurrentAfterLock}
            />
          ) : showFiBoardOddsTrail ? (
  ...
  marketingPreviewGame({
    ...
    markets: {
      ...
      first_inning: marketingPreviewMarket({
        ...
        oddsTrail: [
          { american: -124, line: 0.5, observedAt: "2026-07-07T18:00:00.000Z", sportsbook: "consensus", source: "line_history", label: "first" },
          { american: -121, line: 0.5, observedAt: "2026-07-07T19:00:00.000Z", sportsbook: "consensus", source: "line_history", label: "move" },
          { american: -118, line: 0.5, observedAt: "2026-07-07T19:30:00.000Z", sportsbook: "consensus", source: "current_line", label: "current" },
        ],
        fiMarketBoard: {
          line: 0.5,
          nrfiAmerican: -118,
        ...
        guidedGuide: "NRFI has a model lean, but the edge is not strong enough for action yet.",
        guidedWatchOut: "The FI price is playable, but the edge needs more separation before it becomes actionable.",
        whyLine: "Driver: modest NRFI model edge with price movement not fighting the read.",
        riskLine: "The first-inning model is close enough to market that small price movement can erase the edge.",
      }),
    },
  }),
  marketingPreviewGame({
