...
export async function GET(request: Request): Promise<Response> {
  return cronHandler(request, "cfb_forward_evidence", async ({ runId }) => {
    ...
    const result = await runCfbForwardEvidenceWriter({ client: supabase, season: boundedInteger(process.env.CFB_FORWARD_SEASON ?? "2026", 2026, 2100, "CFB_FORWARD_SEASON"), runId, now: new Date().toISOString(), apply: true, balldontlieApiKey: requiredEnv("BALLDONTLIE_API_KEY"), playbookApiKey: requiredEnv("PLAYBOOK_API_KEY") });
