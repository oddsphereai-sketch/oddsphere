  ...
  return <section id="prop-research-cockpit" data-product-zone="research-cockpit" aria-label="Prop Research Cockpit" className="overflow-hidden rounded-lg border border-gray-800 bg-[#101319]">
    ...
    {selected ? <><div className="flex gap-1 overflow-x-auto border-b border-gray-800 bg-black/20 px-3" data-research-module="selection-list">{matches.length ? matches.map((row) => <button key={row.id} type="button" onClick={() => onResearchSelect(row.id)} aria-pressed={selected.id === row.id} className={`min-w-[170px] border-b-2 px-3 py-3 text-left ${selected.id === row.id ? "border-emerald-400 bg-white/[0.03]" : "border-transparent"}`}><strong className="block truncate text-xs text-white">{row.player}</strong><span className="mt-0.5 block truncate text-[10px] text-gray-500">{row.marketLabel} · {signed(row.odds)}</span></button>) : <p className="px-4 py-6 text-center text-sm text-gray-500">No props match this search.</p>}</div>
      <div className="grid min-w-0 xl:grid-cols-[minmax(0,1fr)_300px]" data-research-module="player-market-snapshot">
        <div className="min-w-0 border-gray-800 p-4 sm:p-6 xl:border-r"><div className="flex flex-col gap-3 border-b border-gray-800 pb-5 sm:flex-row sm:items-end sm:justify-between"><div className="flex min-w-0 items-center gap-3"><PlayerAvatar player={selected.player} team={selected.team} headshotUrl={selected.headshotUrl} /><div className="min-w-0"><p className="text-[10px] font-black uppercase text-emerald-300">Selected prop</p><h3 className="mt-1 truncate text-2xl font-black text-white">{selected.player}</h3><p className="mt-1 text-sm text-gray-500">{selected.marketLabel} · {selected.team} {selected.homeAway === "home" ? "vs" : "@"} {selected.opponent}</p></div></div><div className="sm:text-right"><p className="text-3xl font-black text-white">{marketSelectionLabel(selected)}</p><p className="mt-1 text-xs font-black text-emerald-300">{selected.book} {signed(selected.odds)}</p></div></div><div className="mt-5" data-research-module="projection-vs-line"><ResearchModuleHeading title={isHomeRunMarket(selected) ? "Home run model read" : selected.projectionSource === "recent_form" ? "Recent form vs line" : "The model case"} />{isHomeRunMarket(selected) ? <MilestoneProbabilityVisual row={selected} /> : <ProjectionVsLineVisual projection={selected.projection} line={selected.line} side={selected.side} label={selected.projectionSource === "recent_form" ? "Recent average" : "Projection"} />}</div><details className="group mt-5 border-t border-gray-800 pt-4"><summary className="flex cursor-pointer list-none items-center justify-between text-xs font-black text-gray-300"><span>More model context</span><span className="text-gray-600 transition-transform group-open:rotate-180">⌄</span></summary><div className="mt-4" data-research-module="model-vs-market"><ResearchModuleHeading title="Model vs Market" /><ModelVsMarketVisual row={selected} /></div><MarketContextSummary row={selected} /></details></div>
...
function MarketPairTable({ pairs, selectedId, onSelect }: { pairs: MarketPair[]; selectedId: string; onSelect: (id: string) => void }) {
  ...
  const projectionLabel = pairs.every((pair) => isHomeRunMarket(pair.primary)) ? "Model read" : pairs.every((pair) => pair.primary.projectionSource === "recent_form") ? "Recent avg" : "Projection";
...
export function PropsTable({ rows, selectedId, onSelect }: { rows: PlayerPropPreviewRow[]; selectedId: string; onSelect: (id: string) => void }) {
  ...
  const projectionLabel = rows.every(isHomeRunMarket) ? "Model read" : rows.every((row) => row.projectionSource === "recent_form") ? "Recent avg" : "Projection";
  return <div className="overflow-hidden rounded-lg border border-gray-800 bg-gray-950">
    ...
    <div className="divide-y divide-gray-800">{rows.map((row) => <div key={row.id} className={`px-3 py-3 hover:bg-gray-900 ${selectedId === row.id ? "bg-violet-400/10" : ""}`}>
      <button type="button" onClick={() => onSelect(row.id)} className="hidden w-full grid-cols-[1.25fr_1.08fr_0.78fr_0.64fr_0.82fr_0.56fr_0.54fr_0.68fr_0.82fr_0.48fr] items-center gap-2 text-left xl:grid">
        ...
        <span className="text-xs font-bold text-white">{marketSelectionLabel(row)}{isRowDirectionSide(row) ? <InlineDirectionTag row={row} kind={isRowDirectionSide(row) ?? "prediction"} /> : null}{row.lockStatus ? <LockStatusBadge lockedAt={row.lockStatus.lockedAt} compact /> : null}</span>
        <span className="text-xs font-bold tabular-nums text-gray-200">{projectionHeadlineValue(row)}</span>
  ...
  return <div data-product-zone="player-workspace" className="mt-5">
    ...
    <div className="mt-3 divide-y divide-gray-800 overflow-hidden rounded-lg border border-gray-800 bg-gray-950">{marketPairs.map((pair) => {
      ...
      return <article key={pair.key} className={`grid gap-3 p-4 ${pair.rows.some((row) => row.id === selectedId) ? "bg-violet-400/10" : "hover:bg-gray-900"} lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1.1fr)_minmax(320px,1.25fr)] lg:items-center`}>
        <div className="flex min-w-0 items-center gap-2"><MarketChip row={pair.primary} /><span className="min-w-0"><strong className="block truncate text-sm text-white">{pair.primary.marketLabel}</strong><span className="block truncate text-xs text-gray-500">{isHomeRunMarket(pair.primary) ? `1+ HR · ${projectionHeadlineLabel(pair.primary)} ${projectionHeadlineValue(pair.primary)}` : `Line ${pair.primary.line} · ${projectionHeadlineLabel(pair.primary)} ${projectionHeadlineValue(pair.primary)}`}</span>{pair.primary.lockStatus ? <LockStatusBadge lockedAt={pair.primary.lockStatus.lockedAt} compact /> : null}</span></div>
...
export function PropDetailDrawer({ row, comparisons, onClose, showDiagnostics = false, researchPending = false }: { row: PlayerPropPreviewRow; comparisons: PlayerPropPreviewRow[]; onClose: () => void; showDiagnostics?: boolean; researchPending?: boolean }) {
  ...
  return <PlayerPropReaderDialog ariaLabel={`${row.player} prop details`} title={row.player} leading={<PlayerAvatar player={row.player} team={row.team} headshotUrl={row.headshotUrl} compact />} onClose={onClose}>
...
export function ProjectionVsLineVisual({ projection, line, side, label = "Projection" }: { projection: number; line: number; side: "over" | "under"; label?: string }) {
  ...
  return <div data-visual="projection-vs-line" data-projection={projection} data-line={line} data-side={side} className="rounded-lg border border-gray-800 bg-black/20 p-3"><div className="flex items-center justify-between gap-3 text-xs"><span className="text-gray-500">{label} <strong className="ml-1 text-white">{formatProjection(projection)}</strong></span><span className="font-bold text-sky-300">{projectionDirection}</span><span className="text-gray-500">Line <strong className="ml-1 text-white">{line}</strong></span></div><div className="relative mt-5 h-2 rounded-full bg-gray-800"><span className="absolute inset-y-0 rounded-full bg-sky-400/25" style={{ left: position(Math.min(line, projection)), right: `${100 - Number.parseFloat(position(Math.max(line, projection)))}%` }} /><span className="absolute -top-1.5 h-5 w-0.5 bg-white" style={{ left: position(line) }} /><span className="absolute -top-1 h-4 w-4 -translate-x-1/2 rounded-full border-2 border-gray-950 bg-sky-400" style={{ left: position(projection) }} /></div><div className="mt-4 flex justify-between text-[9px] text-gray-600"><span>{low.toFixed(1)}</span><span>{high.toFixed(1)}</span></div></div>;
}

function MilestoneProbabilityVisual({ row }: { row: PlayerPropPreviewRow }) {
  const chance = homeRunProbability(row);
  return <div data-visual="milestone-probability" data-probability={chance ?? ""} className="rounded-lg border border-gray-800 bg-black/20 p-3">
    <div className="grid grid-cols-3 gap-3 text-center">
      <div><p className="text-[9px] font-bold uppercase text-gray-600">Chance of 1+ HR</p><strong className="mt-1 block text-xl tabular-nums text-white">{chance === null ? "-" : pct(chance)}</strong></div>
      <div className="border-x border-gray-800"><p className="text-[9px] font-bold uppercase text-gray-600">Fair price</p><strong className="mt-1 block text-xl tabular-nums text-white">{row.fairOdds === null ? "-" : signed(row.fairOdds)}</strong></div>
      <div><p className="text-[9px] font-bold uppercase text-gray-600">Best price</p><strong className="mt-1 block text-xl tabular-nums text-emerald-300">{signed(row.odds)}</strong></div>
    </div>
    <div className="mt-4 h-2 overflow-hidden rounded-full bg-gray-800">{chance !== null ? <span className="block h-full rounded-full bg-violet-400" style={{ width: `${Math.round(clamp01(chance) * 100)}%` }} /> : null}</div>
  </div>;
}
...
function isHomeRunMarket(row: PlayerPropPreviewRow): boolean {
  return row.market === "batter_home_runs";
}

function homeRunProbability(row: PlayerPropPreviewRow): number | null {
  if (!isHomeRunMarket(row)) return null;
  if (row.side === "over") return row.finalProbability ?? row.overProbability ?? row.modelProbability;
  return row.overProbability ?? (row.finalProbability === null ? null : 1 - row.finalProbability);
}

function projectionHeadlineLabel(row: PlayerPropPreviewRow): string {
  if (isHomeRunMarket(row)) return "Model pick";
...
function projectionHeadlineValue(row: PlayerPropPreviewRow): string {
  ...
  return isPositiveSignal(row) ? "1+ Home Run" : "No HR pick";
}

function marketSelectionLabel(row: PlayerPropPreviewRow): string {
  return isHomeRunMarket(row) ? "1+ Home Run" : `${row.side === "over" ? "Over" : "Under"} ${row.line}`;
}
...
function propReaderSummary(row: PlayerPropPreviewRow, prices: PlayerPropPreviewRow[]): string {
  ...
  return `${row.player}'s ${formatProjection(row.projection)} ${estimateLabel} sits ${Math.abs(delta).toFixed(1)} ${direction} the ${row.line} line. ${integrity} ${probability} ${price}${pricePolicy}${longshotContext}`;
}
...
function cardReason(row: PlayerPropPreviewRow): string {
  ...
  if (row.reasonCodes.includes("LONGSHOT_VALUE_CONTEXT")) return isPositiveSignal(row)
    ...
    : "The model price is better than the market, but this remains a non-actionable rare-event read.";
