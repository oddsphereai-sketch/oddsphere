...
function LockStatusBadge({ lockedAt, compact = false }: { lockedAt: string; compact?: boolean }) {
  return <span className={`inline-flex w-fit items-center gap-1 rounded border border-emerald-400/40 bg-emerald-400/10 font-black uppercase text-emerald-200 ${compact ? "px-1.5 py-0.5 text-[8px]" : "px-2 py-1 text-[9px]"}`} title={`Locked at ${formatTime(lockedAt)}`}>
