  ...
  return <nav aria-label="Player props leagues" className="-mx-4 mb-5 border-y border-white/[0.05] bg-white/[0.015] px-4 py-2.5 sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
    <div className="flex gap-2 overflow-x-auto pb-1">
      {leagues.map((item) => {
        ...
        return item.href
          ? <PlayerPropsLeagueLink key={item.key} href={item.href} prefetchOnMount={!active} aria-current={active ? "page" : undefined} aria-label={`${item.label} player props${active ? ", active" : ""}`} className={className}>{contents}</PlayerPropsLeagueLink>
