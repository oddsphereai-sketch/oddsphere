...
| F2 | P0 | HIGH | NHL | Puck-line displayed in `first_inning` slot without internal capture OR context-only label | LIVE: members see a recommendation that has no audit trail and looks tracked | `marketKeysFor` in `DailyEdgeShell.tsx`; `buildNhlPredictionRecords.ts:7-11` | snapshot_json | Choose Option A (`displayed_market_snapshot` + UI label) or Option B (hide via `marketKeysFor`) | NO | YES (recommend Option A) | YES | MEDIUM (touches live UI) |
...
| S4 | No displayed market lacks either official tracking OR context-only labeling + internal snapshot | NBA spread: hidden (Option B). NHL puck-line: Option A (captured) or Option B (hidden). No "tracked-looking" market without audit trail |
