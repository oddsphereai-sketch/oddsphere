...
- Forward evidence schema / collector / writer: `nfl_forward_evidence_snapshot_2026_08_22_r2_multibook` / `nfl_forward_evidence_collector_2026_08_22_r2_multibook` / `nfl_forward_evidence_writer_2026_08_22_r3_r6_shadow`
- Internal moneyline shadow releases: runtime artifact `nfl_r6_moneyline_runtime_artifact_2026_08_22_r1`, model `nfl_market_led_moneyline_shadow_2026_08_22_r6`, calibration `nfl_market_led_price_calibration_shadow_2026_08_22_r6`, and decision `nfl_market_led_moneyline_lean_shadow_2026_08_22_r6`. These are forward-evaluation releases only; they are not member-facing model/grade releases.
- Candidate member outcome releases: artifact `nfl_v1_week_one_outcome_artifact_2026_08_23_r1`, independent point model `nfl_v1_comprehensive_outcome_2026_08_23_r1`, joint distribution `nfl_v1_bivariate_score_distribution_2026_08_23_r1`, and win probability `nfl_v1_beta_calibrated_win_ensemble_2026_08_23_r1`. These publish score and winner forecasts only; they do not authorize any exact-price Bet grade, stake, tracking row, or spread/total probability.
...
- Current member state: the genuine 16-game 2026 Regular Season Week 1 schedule with 16/16 complete current FanDuel moneyline/spread/total headline boards, 16/16 immutable operational Opening trails, 16/16 Playbook public-consensus sets, 16/16 injury/depth snapshots, and all 32 expected quarterbacks. The r2 forward collector also preserves every contemporaneous BALLDONTLIE book row and separately stamps the conventional sportsbook subset for future leave-one-book-out research; Kalshi and Polymarket remain observable but are excluded from that comparable subset. The August 22 no-write probe found seven complete rows per game and five comparable conventional sportsbooks per game. Expected quarterbacks remain projected rather than confirmed, SharpAPI has zero strictly matched Week 1 split sets, and outdoor weather is collected only inside its forecast window.
- Prediction boundary: the candidate Week 1 reader publishes the frozen football-only score and winner forecast separately from the market decision. The independent tournament passed its predeclared football-baseline accuracy, calibration, interval-coverage, and Week 1 dispersion gates; it did not beat the opening market and is labeled independent rather than market-superior. Exact-price Bet grades, stakes, tracking rows, spread/total probabilities, and settlement instructions remain Held. The r3 writer stores the r6 moneyline candidate under a separate `shadowEvaluatedBets` array with `shadowOnly=true`, `publicationEligible=false`, and `trackingEligible=false`; production `evaluatedBets` and `outcomeConfidence` remain empty. Market prices and public splits remain evidence only and cannot masquerade as OddSphere predictions.
- Board impact: the expired August 20 preseason display is removed as a phase transition, not regraded. Its remaining three rehearsal Leans, five Watchlists, and 25 No Plays are not promoted, demoted, settled, or imported into regular-season tracking. Replaying the latest authoritative 16-game r2 evidence through the exact r3 runtime produces nine internal moneyline Lean candidates and seven Held evaluations. Relative to the current production-held state that is nine proposed shadow promotions and zero shadow demotions, but applied member promotions/demotions remain zero/zero and the Week 1 evidence board issues zero grades.
...
- Refresh boundary: the scheduled `/api/cron/nfl-forward-evidence` route remains the only writer, owns the shared `prediction_pipeline:nfl` lease, and captures opening, six-hour early, hourly inside 48 hours, and T-60 evidence. r2 stores the already-returned all-book and comparable-book arrays without another provider request, seeds one r2 opening row per game while carrying forward the earlier r1 operational-opening quote and timestamp, then resumes the existing cadence. r3 recomputes one coherent r6 moneyline shadow tuple from each due capture's exact target price and leave-one-book-out conventional-sportsbook consensus without another provider call. A T-60 tuple freezes only when its actual and reported capture lag agree and are both at most 20 minutes; otherwise it is Held and remains unfrozen. Member reads query stored evidence and make zero provider calls.
...
- Evidence: `docs/model-audits/2026-08-22-nfl-week1-evidence-board-r1.md`, `docs/model-audits/2026-08-22-nfl-multibook-forward-evidence-r2.md`, `docs/model-audits/2026-08-22-nfl-r6-shadow-writer.md`, and `docs/model-audits/2026-08-23-nfl-v1-comprehensive-outcome.md`. The score/winner release is separate from the still-shadow r6 Bet lane. Exact-price publication remains blocked until timestamp-valid T-60, CLV, settlement, quarterback health, and owner approval gates pass.
