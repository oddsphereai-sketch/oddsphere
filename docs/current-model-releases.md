...
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_28_r3_display_quote_coverage`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r7_expanded_slate_budget`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r7_display_quote_coverage` / `cfb_forward_evidence_collector_2026_08_28_r14_expanded_sharp_budget` / `cfb_v1_member_release_2026_08_28_r14_expanded_sharp_budget` / `cfb_forward_evidence_writer_2026_08_28_r17_distribution_tossup`. Member fixture release is `cfb_v1_member_fixture_2026_08_28_r18_expanded_sharp_budget`. The append-only table schema is unchanged. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, unlocked, and T-60 evidence; no parallel timer or endpoint was added. It consumes every named-book observation returned by BALLDONTLIE for display coverage and the strict SharpAPI exact-event fallback, while only the established six supported target books and qualified comparison cohort can grade. Complete pairs stay in `currentBooks`; verified one-sided main offers stay in `displayBooks` and can appear only as sportsbook context after a representative-market filter rejects Moneyline decimal-odds outliers and Spread/Total line outliers against the cross-book/Playbook center. Before its sole all-game append, writer r17 applies shared coherence release `football_cross_market_coherence_2026_08_28_r2_distribution_tossup`: a checkable joint/marginal distribution may cross the expected-margin/winner-probability sign only inside the half-point/one-percentage-point toss-up zone. PMF identity, representative winner, market count, exact quote, side, EV, actionable value, and push-aware Moneyline/Spread containment remain fatal. A valid exact-price grade difference is retained and the member explanation states the exact price/EV reason; the gate does not force grades to match. Strict price pagination remains forward-only and bounded. When SharpAPI claims more exact-event rows but omits `next_offset`, r7 advances by the provider-reported positive page limit after verifying the reported current offset; returned row count is only a compatibility stride when no valid limit exists. Exact candidates try the Eastern football date before UTC for evening kickoffs, and the expanded Division I slate uses a 192-request all-run hard cap; repeated, empty, conflicting, non-advancing, over-cap, or malformed pages still fail before the all-game append. Paired-alternate admission remains market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. A complete Circa row has first priority; until it is published, a complete DraftKings row occupies the established Sharp Book Splits member card and is replaced automatically by Circa. Playbook remains the separate Public Consensus card. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
...
Shared member presentation release: `daily_edge_member_presentation_2026_08_28_r13_single_sharp_book_card`.
