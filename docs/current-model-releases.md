...
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_27_r7_market_prediction_contract` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` scope the normal Daily Edge slate/reader to the active weekly window and keep the latest complete release authoritative. The common route, DTO types, reader shell, sport switching, modal behavior, and presentation components remain shared with mature MLB behavior. During transition it serves a complete legacy/prior wave until the sole writer atomically appends a complete r3/r5/r9 wave; it never mixes releases. Compact movement remains chronological and same-book. Every internal exception preserves the independent outcome forecast and score context; only its evaluated sportsbook side/price, fair probability, edge, EV, actionability, grade price, and tracking proposal are absent. A separate market-prediction DTO now carries the coherent PMF side at the exact named-book line, or at a fresh same-capture Playbook consensus line when the named book is unavailable. Exact named-book and consensus provenance remain distinct; the reader explicitly labels consensus as context rather than an available sportsbook offer, and consensus never supplies a missing Bet-grade price. Spread/Total prediction surfaces report an explicit market-prediction failure when neither source exists and never substitute projected margin/total for a bettable side and line. The shared presentation boundary renders that Bet grade as a reasoned No Play while internal recovery remains active. `CFB_DAILY_EDGE_ENABLED=true` and `CFB_FORWARD_EVIDENCE_ENABLED=true` retain their existing gates. Rollback is decision r7 / tuple r2 / evidence r2 / member r4 / writer r6 / fixture r6 / Sharp fallback r1; immutable evidence and tracking remain release-scoped.
