...
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_28_r3_display_quote_coverage`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r9_event_discovery_pagination`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r10_event_discovery_pagination` / `cfb_forward_evidence_collector_2026_08_28_r16_event_discovery_pagination` / `cfb_v1_member_release_2026_08_28_r18_event_discovery_pagination` / `cfb_forward_evidence_writer_2026_08_28_r21_event_discovery_pagination`. Member fixture release is `cfb_v1_member_fixture_2026_08_28_r22_event_discovery_pagination`. The append-only table schema is unchanged, and the complete r29 member wave remains the atomic fallback until one complete current-member wave exists. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, unlocked, and T-60 evidence; no parallel timer or endpoint was added. It consumes every named-book observation returned by BALLDONTLIE for display coverage and the strict SharpAPI exact-event fallback, while only the established six supported target books and qualified comparison cohort can grade. Complete pairs stay in `currentBooks`; verified one-sided main offers stay in `displayBooks` and can appear only as sportsbook context after a representative-market filter rejects Moneyline decimal-odds outliers and Spread/Total line outliers against the cross-book/Playbook center. Before its sole all-game append, writer r21 applies strict shared coherence release `football_cross_market_coherence_2026_08_28_r4_public_prediction_side` to the independent PMF. PMF identity, score/winner direction, representative winner, exact-line PMF/score/decision side, market count, exact quote, EV, actionable value, and push-aware Moneyline/Spread containment are fatal; only a mean difference within the frozen 0.25-point discrete quantization zone defers to the full PMF direction. A valid exact-price grade difference is retained and the member explanation states the exact price/EV reason; the gate does not force grades to match. Sharp r9 first discovers canonical NCAAF events once per unique Eastern/UTC slate date, following up to eight forward-only pages per date before requiring one exact home/away/time match and requesting `market=main` odds for that event ID. It never constructs or probes guessed event bucket IDs. The per-date page stop remains inside the unchanged 192-request all-run hard cap; zero event matches remain unavailable, while repeated pages, non-advancing offsets, ambiguity, canonical-event reuse, malformed data, or an exhausted cap fail before the all-game append. Paired-alternate admission remains market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. A complete Circa row has first priority; until it is published, a complete DraftKings row occupies the established Sharp Book Splits member card and is replaced automatically by Circa. Playbook remains the separate Public Consensus card. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. The immutable capture/evaluation timestamp is the latest real provider observation included in each payload. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
