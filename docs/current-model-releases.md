...
- Reader-axis patch retained by shared presentation release `daily_edge_member_presentation_2026_08_28_r12_sharp_book_split_priority` keeps the prediction tab label and probability attached to `marketPrediction`, while the Quick Read exact-price panel keeps the evaluated side, book, price, selected-side probability, and Bet Grade attached to `pick`. Moneyline prediction copy now says the exact-price grade is separate rather than incorrectly implying that a visible sportsbook quote is missing. MLB/NFL/CFB retain the familiar Public Consensus and Sharp Book Splits cards; a complete named-book fill-in occupies the latter until valid Circa data replaces it automatically, while typed payload provenance retains the actual source internally. This changes zero model outputs, exact tuples, grades, stakes, locks, or tracking rows.
...
Shared member presentation release: `daily_edge_member_presentation_2026_08_28_r12_sharp_book_split_priority`.
