...
- Current authoritative replay: eight opening-week games, 24 markets, and 21 complete exact-price decisions produce **1 Best Angle / 2 Leans / 10 Watchlists / 8 No Plays / 3 Held**. The three Held markets are all SJSU-USC because a coherent named-book tuple is absent; its independent outcome forecast remains published. Expected team points span 16.08-39.42 (SD 7.26), margins -7.82 to +23.34 (SD 9.68), totals 46.58-58.08 (SD 3.23), and the board is four Over/four Under. Decimal expected points are primary; the secondary central reachable score comes from the identical PMF and must preserve winner fidelity and centrality.
- Writer and provider boundary: evidence schema / collector / member / writer are `cfb_forward_evidence_snapshot_2026_08_25_r1` / `cfb_forward_evidence_collector_2026_08_25_r2_team_scoped_qb` / `cfb_v1_member_release_2026_08_25_r1` / `cfb_forward_evidence_writer_2026_08_25_r2_team_scoped_qb`. One scheduled writer owns the `prediction_pipeline:cfb` lease and captures immutable Opening/T-60 evidence plus bounded unlocked refreshes. BALLDONTLIE supplies the authoritative slate, named-book prices, team-scoped active rosters, and previous-season QB passing context. The collector requests each of the 16 exact slate teams separately, allows at most two roster pages per team and two QB-stat pages, rejects any off-team/off-player response, and aborts before the single append if the bounded context is incomplete. Playbook public consensus is labeled separately. Strict SharpAPI NCAAF splits, timestamped injuries, confirmed depth/starters, and venue weather are currently unavailable and are never fabricated.
...
- Reader/rollback: `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` keeps the weekly CFB board visible through each game's Eastern date and removes prior-date games without touching stored lock/tracking history. `CFB_DAILY_EDGE_ENABLED=true` selects the normal Daily Edge slate cards and selected-edge reader. `CFB_FORWARD_EVIDENCE_ENABLED=true` enables the sole writer. A real per-market health problem renders Held within the normal product. Disabling either gate stops new publication or hides the sport without deleting immutable evidence or tracking records.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the two versioned JSON model artifacts. Evidence: `docs/model-audits/2026-08-25-cfb-v1-composite-grade-release.md` plus the frozen CFB v1 predeclarations in the same directory.
