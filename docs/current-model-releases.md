...
- Evidence: `docs/model-audits/2026-08-25-nfl-odds-history-reader-repair.md`, `docs/model-audits/2026-08-25-nfl-public-release-transition.md`, `docs/model-audits/2026-08-25-nfl-actionable-grades-production-r9.md`, `docs/model-audits/2026-08-25-nfl-actionable-grades-r9.md`, `docs/model-audits/2026-08-25-nfl-projected-qb-context-r11.md`, `docs/model-audits/2026-08-23-nfl-discrete-drive-joint-r10.md`, and `docs/model-audits/2026-08-23-nfl-v1-comprehensive-outcome.md`.
...
- Active grade / decision releases: `cfb_v1_composite_grade_policy_2026_08_25_r1` / `cfb_v1_daily_edge_decision_2026_08_25_r5_weekly`. Every grade evaluates one named sportsbook/line/price against target-excluded same-line named-book consensus. Moneyline and Total have qualified Best Angle lanes; Spread is capped at Lean. Watchlist is a complete positive tuple below the Lean boundary; No Play is a complete non-positive tuple; missing identity, price, consensus, PMF, or T-60 health is Held. There is no quota, forced minimum, maximum, stake sizing, or reader override. The r5 decision release changes coverage/runtime identity only; coefficients, calibration, thresholds, sides, and Week 0 grades are unchanged.
...
- Current authoritative Week 0 replay remains eight games, 24 markets, and 21 complete exact-price decisions producing **1 Best Angle / 2 Leans / 10 Watchlists / 8 No Plays / 3 Held**: zero promotions and zero demotions from r4. The three Held markets are all SJSU-USC because a coherent named-book tuple is absent; its independent outcome forecast remains published. Expected team points span 16.08-39.42 (SD 7.26), margins -7.82 to +23.34 (SD 9.68), totals 46.58-58.08 (SD 3.23), and the board is four Over/four Under. Decimal expected points are primary; the secondary central reachable score comes from the identical PMF and must preserve winner fidelity and centrality.
...
- Writer and provider boundary: evidence schema / collector / member / writer are `cfb_forward_evidence_snapshot_2026_08_25_r1` / `cfb_forward_evidence_collector_2026_08_25_r3_weekly` / `cfb_v1_member_release_2026_08_25_r2_weekly` / `cfb_forward_evidence_writer_2026_08_25_r3_weekly`. One scheduled writer owns the `prediction_pipeline:cfb` lease and captures immutable Opening/T-60 evidence plus bounded unlocked refreshes. BALLDONTLIE supplies the authoritative weekly slate, named-book prices, team-scoped active rosters, and previous-season QB passing context. QB collection prioritizes T-60 and earliest missing teams, requests no more than 24 team identities per run (at most two roster pages each plus two stat pages), and reuses prior immutable team context; any provider/budget/identity failure aborts before the single append. Teams outside the portable model identity map retain a neutral-imputed independent forecast but their exact-price markets fail closed. Playbook public consensus is labeled separately. Strict SharpAPI NCAAF splits, timestamped injuries, confirmed depth/starters, and venue weather are currently unavailable and are never fabricated.
- Official tracking: `cfb_official_tracking_record_2026_08_25_r1_t60` begins forward-only on the 2026-08-29 ET slate. Only a complete exact-price tuple frozen at T-60 no more than 20 minutes late can enter `prediction_records`; unlocked grades are never counted. No historical backfill is authorized.
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_25_r2_weekly` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` scope the normal Daily Edge slate/reader to the active weekly window, keep each game visible through its Eastern date, and remove prior-date games without touching stored lock/tracking history. `CFB_DAILY_EDGE_ENABLED=true` selects the normal Daily Edge slate cards and selected-edge reader. `CFB_FORWARD_EVIDENCE_ENABLED=true` enables the sole writer. A real per-market health problem renders Held within the normal product. Disabling either gate stops new publication or hides the sport without deleting immutable evidence or tracking records.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbV1WeeklyForecast.ts`, `lib/services/football/cfbWeeklyWindow.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the three versioned JSON model artifacts. Evidence: `docs/model-audits/2026-08-25-cfb-generalized-weekly-engine-predeclaration.md`, `docs/model-audits/2026-08-25-cfb-v1-composite-grade-release.md`, plus the frozen CFB v1 predeclarations in the same directory.

## NFL Player Props production candidate (private approval hold)

- Candidate releases: model `nfl_player_props_distribution_model_2026_08_25_r2_shared_context`; calibration `nfl_player_props_distribution_calibration_2026_08_25_r2_shared_context`; decision `nfl_player_props_decision_2026_08_25_r2_exact_price_shared_context`; runtime `nfl_player_props_runtime_2026_08_25_r2_shared_context`; member snapshot `nfl_player_props_member_2026_08_25_r4_shared_context_load_bound`; writer `nfl_player_props_writer_2026_08_25_r5_bounded_settlement`; tracking `nfl_player_props_tracking_2026_08_25_r4_regular_t60_shared_context`; settlement `nfl_player_props_settlement_2026_08_25_r3_bounded_finality`.
- Status: production-qualified but unpublished. The qualified source artifact's historical `*_shadow_*` identifiers remain checksum provenance only and are never emitted by the production scorer, board, member snapshot, or tracking rows. `NFL_PLAYER_PROPS_ENABLED` and `NFL_PLAYER_PROPS_MEMBER_ENABLED` remain exact-string opt-in gates. Publication is held for owner approval of the private MLB-style member design; no PR, deployment, database write, or official tracking activation has occurred.
...
- Current exact replay: the real timestamped 2026 Week 1 input has 2,148 offers and produces **1 Best Angle / 0 Leans / 10 Watchlists / 21 No Plays / 2 Held**, with 335 no-independent-benchmark outcomes excluded as unavailable and 264 incomplete raw offers excluded before grading. The Best Angle is Christian McCaffrey receiving yards Under 36.5 at NoVig +117: 29.75 projection, 53.75% calibrated probability, +3.75pp edge, +16.63% exact-price EV, and 92.08% participation. Versus the prior private runtime this is one actionable promotion, zero actionable demotions, and 335 availability rows correctly removed from Held semantics.
...
- Reader/rollback: the private route is `/dev/nfl-props-preview`; the normal member route remains gated. Roll back by leaving both props gates disabled or reverting to the preserved private-review checkpoint `f57188f3`; already locked records remain immutable.
...
- Evidence: `docs/model-audits/2026-08-25-nfl-player-props-production-candidate-r4.md`.
