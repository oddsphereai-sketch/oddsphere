...
- Current SELECT-only replay covers 38 games / 114 markets from 332 immutable rows. It retains 27 exact tuples: **3 Best Angles / 2 Leans / 10 Watchlists / 12 evaluated No Plays**, with 87 market-scoped unavailable states. The r29 public-prediction change produces zero tuple changes, zero promotions, and zero demotions. All 38 games pass the strict independent-PMF coherence gate. SJSU-USC now publishes the independent Under 61.5 prediction at the identical FanDuel line beside its Under 61.5 Best Angle, while Moneyline remains an honest price-unavailable No Play. Hawaii-Stanford publishes the independent Hawaii winner/score prediction and its Hawaii exact-price side without the former Stanford market-anchor score. Virginia-NC State and UC Davis-Portland State retain their released independent PMF directions. The only mean/PMF line cross is MEM-UNLV Spread at 0.1142 points from the line, inside the predeclared 0.25-point discrete-score quantization zone; there are zero genuine same-line direction conflicts. Strict Sharp splits match 2/38 games and unmatched rows never render.
...
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_28_r3_display_quote_coverage`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r10_ambiguous_event_scope`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r10_event_discovery_pagination` / `cfb_forward_evidence_collector_2026_08_28_r17_ambiguous_event_scope` / `cfb_v1_member_release_2026_08_28_r19_ambiguous_event_scope` / `cfb_forward_evidence_writer_2026_08_28_r23_release_refresh_priority`. Member fixture release is `cfb_v1_member_fixture_2026_08_28_r23_ambiguous_event_scope`. The append-only table schema is unchanged, and the complete r31 wave is the first atomic fallback, followed by the complete r29 wave, until one complete current-member wave exists. If the current release is incomplete for any pre-T60 upcoming game, that release refresh takes planning priority over ordinary cadence/T-60 reasons; the same planner still gives a due game its T-60 stage and the remaining upcoming games unlocked stages. This prevents one due game from starving the complete current-release wave. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, six-hour distant, hourly inside-24-hours, and event-triggered T-60 evidence; the scheduled 15-minute heartbeat is only an eligibility check and makes no provider call or write when cadence/T-60 work is not due. It consumes every named-book observation returned by BALLDONTLIE for display coverage and the strict SharpAPI exact-event fallback, while only the established six supported target books and qualified comparison cohort can grade. Complete pairs stay in `currentBooks`; verified one-sided main offers stay in `displayBooks` and can appear only as sportsbook context after a representative-market filter rejects Moneyline decimal-odds outliers and Spread/Total line outliers against the cross-book/Playbook center. Before its sole all-game append, writer r23 applies strict shared coherence release `football_cross_market_coherence_2026_08_28_r4_public_prediction_side` to the independent PMF. PMF identity, score/winner direction, representative winner, exact-line PMF/score/decision side, market count, exact quote, EV, actionable value, and push-aware Moneyline/Spread containment are fatal; only a mean difference within the frozen 0.25-point discrete quantization zone defers to the full PMF direction. A valid exact-price grade difference is retained and the member explanation states the exact price/EV reason; the gate does not force grades to match. Sharp r10 first discovers canonical NCAAF events once per unique Eastern/UTC slate date, following up to eight forward-only pages per date before requiring exact home/away/time identity and requesting `market=main` odds for one unique event ID. It never constructs or probes guessed event bucket IDs. If one scheduled game has multiple exact canonical IDs, it requests odds from neither and marks that game's fallback evidence ambiguous without aborting its unambiguous siblings; one canonical ID reused across different games remains fatal. The per-date page stop remains inside the unchanged 192-request all-run hard cap; repeated pages, non-advancing offsets, malformed data, or an exhausted cap remain fatal before the all-game append. Paired-alternate admission remains market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. A complete Circa row has first priority; until it is published, a complete DraftKings row occupies the established Sharp Book Splits member card and is replaced automatically by Circa. Playbook remains the separate Public Consensus card. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. The immutable capture/evaluation timestamp is the latest real provider observation included in each payload. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
...
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_28_r23_ambiguous_event_scope`, public outcome contract `cfb_independent_public_outcome_contract_2026_08_28_r29`, and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` keep the mature MLB Daily Edge shell: weekly slate cards, selected-edge interaction, shared three-column hierarchy, responsive/mobile behavior, exact price/movement panels, common grade ladder, and expandable football evidence. Quick Read shows exactly one independent expected score/winner/reachable final. At an evaluated line, Spread and Total predictions use the independent PMF at the identical book line; Bet probability, exact target price, fair probability, EV, thresholds, grade, and lock remain separate. The market-informed shadow is not rendered as a score or prediction. The reader retains exactly one Public Consensus card and one Sharp Book Splits card. Missing complete price evidence is member-facing No Play, never a Held prediction; the independent forecast remains visible and the internal recovery flag remains machine-readable. Up to five prioritized football drivers remain visible and all additional supplied rows remain reachable. `CFB_DAILY_EDGE_ENABLED=true` and `CFB_FORWARD_EVIDENCE_ENABLED=true` retain their gates. Roll back member/writer to r18/r21 and fixture to r22 if the repaired natural wave does not complete coherently; presentation remains r16.
- Shared presentation release `daily_edge_member_presentation_2026_08_28_r16_cfb_independent_public_prediction` keeps the prediction tab label/probability attached to the independent PMF and the Quick Read exact-price panel attached to the evaluated book/line/price and calibrated Bet probability. An explicit unavailable football Spread or Total prediction uses the existing unavailable label instead of presenting raw projected margin or total as a line-specific prediction; expected score and independent winner context remain visible. The member split surface renders no duplicate Sharp Book panels. This changes zero exact-price tuples, probability formulas, grade thresholds, stakes, locks, or tracking rules.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbV1WeeklyForecast.ts`, `lib/services/football/cfbMarketInformedOutcome.ts` (shadow market context only), `lib/services/football/footballCrossMarketCoherence.ts`, `lib/services/football/cfbWeeklyWindow.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbSharpApiOdds.ts`, `lib/services/football/cfbSharpApiSplits.ts`, `lib/services/football/footballMarketScopedTracking.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the versioned score/weekly/grade/market-residual JSON artifacts. Evidence includes `docs/model-audits/2026-08-28-cfb-event-discovery-pagination-r31-predeclaration.md`, `docs/model-audits/2026-08-28-cfb-event-discovery-pagination-r31.md`, `docs/model-audits/2026-08-28-cfb-independent-public-prediction-r29-predeclaration.md`, and the prior CFB model, price, tracking, split, weekly-window, and directional-PMF audits.
