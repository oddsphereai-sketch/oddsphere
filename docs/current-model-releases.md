...
- Writer/releases: evidence schema / collector remain `nfl_forward_evidence_snapshot_2026_08_23_r3_member` / `nfl_forward_evidence_collector_2026_08_23_r3_member`; the single authoritative leased writer is `nfl_forward_evidence_writer_2026_08_28_r11_cross_market_coherence`. It remains the only prediction writer under `prediction_pipeline:nfl`; no parallel endpoint or reader-side override exists. Writer r11 preserves r10 compact-snapshot behavior and r9 market-scoped T-60 behavior, then runs the shared score, probability, side, quote, EV, actionable-value, market-count, and event-containment assertion on every planned payload before the sole append. It packages the exact release-keyed member fixture into `nfl_forward_member_snapshot_2026_08_27_r1_compact` after the raw append, or refreshes that compact row on a natural cadence-not-due cycle without a provider call. The indexed row is a read optimization only: append-only raw evidence remains authoritative, a release/checksum guard rejects mixed payloads, and the reader falls back to the raw builder if the compact row is absent or invalid. When an upcoming pre-T60 game's latest row does not contain exactly three current r10 decisions under the current r7 member release, the next natural scheduled cycle still performs one provider-backed unlocked refresh for the full slate instead of waiting for the ordinary six-hour cadence or relabeling old quotes. T-60 work retains priority, and completed/no-unlocked slates do not generate release-transition provider calls. Strict NFL SharpAPI rows remain distinct from Playbook public consensus.
...
- Reader/rollback: member fixture `nfl_week_one_member_fixture_2026_08_28_r9_primary_prediction_authority` consumes every immutable current-schema capture for the exact evaluated sportsbook instead of discarding all but the latest row. Its `marketPrediction` fields are recomputed only from the stored joint outcome distribution at the displayed line; the separately calibrated exact-price selection remains `pick` and cannot overwrite the prediction. The writer-owned compact row stores that already-built fixture behind one indexed exact-key read; the server keeps only a 15-second private cache, and live sport-tab pointer/focus intent prefetches the canonical member route without changing the URL or selection. Missing/invalid compact data falls back to the same raw builder, so the optimization cannot alter price, forecast, grade, stake, lock, writer, or tracking behavior. `NFL_WEEK_ONE_EVIDENCE_BOARD_ENABLED=true` selects the standard 16-game Week 1 slate/reader; `NFL_DAILY_EDGE_ENABLED` remains the visibility rollback. Incomplete health presents a reasoned public No Play on the Bet-grade axis while the independent outcome forecast remains visible for every exception class; the member reader never labels a prediction Held. Evaluated bet side/book/price, fair probability, edge, EV, and actionability remain absent; internal recovery remains active. Disable the publication gates to roll back member visibility; immutable evidence and already-locked official records remain preserved.
- Reader-axis patch: shared presentation release `daily_edge_member_presentation_2026_08_28_r10_football_prediction_bet_axis` keeps the prediction tab label and probability attached to `marketPrediction`, while the Quick Read exact-price panel keeps the evaluated side, book, price, selected-side probability, and Bet Grade attached to `pick`. When those qualified heads disagree, the reader names both sides instead of combining one side's label with the other side's probability or quote. This changes zero model outputs, exact tuples, grades, stakes, locks, or tracking rows.
...
- Active public outcome contract: `cfb_market_informed_outcome_contract_2026_08_28_r18`, using artifact / model / distribution / probability / representative-score releases `cfb_market_anchored_joint_score_artifact_2026_08_27_r4` / `cfb_market_informed_joint_score_model_2026_08_27_r2` / `cfb_market_residual_joint_distribution_2026_08_27_r2` / `cfb_market_informed_joint_probability_2026_08_27_r2` / `cfb_market_informed_reachable_score_2026_08_27_r2`. The primary one-decimal expected score, winner probability, reachable final, margin/total ranges, and every member-facing directional prediction come from that single joint PMF even when sportsbook evidence is incomplete. Prediction surfaces never inherit a Bet selection, grade, or provider-availability state. Its canonical mean is the median of at least three complete conventional named books; when the main cohort is sparse, one supported target book may anchor only when its exact Spread and Total lines are each corroborated by at least two distinct conventional non-target books. Playbook may remain a clearly labeled final context fallback, never a target price. The frozen residual artifact contains 1,111 genuine 2021-23 generic-pregame residual pairs mirrored to 2,222 outcomes. The prior football-only artifact / model / distribution / probability / representative-score releases remain `cfb_v1_joint_score_artifact_2026_08_25_r3_weekly` / `cfb_v1_independent_score_model_2026_08_25_r1` / `cfb_v1_empirical_joint_score_distribution_2026_08_25_r1` / `cfb_v1_joint_market_probability_2026_08_25_r1` / `cfb_v1_central_reachable_score_2026_08_25_r1` and remain visible as the secondary independent baseline. That head uses leakage-safe football inputs and excludes market line, price, fair probability, splits, and movement.
- Active grade / decision releases: `cfb_v1_composite_grade_policy_2026_08_25_r1` / `cfb_v1_daily_edge_decision_2026_08_28_r11_market_scoped_data_quality` using tuple schema `cfb_v1_exact_price_decision_tuple_2026_08_28_r5_structured_unavailable_reasons`. Exact-price decisions continue to use the unchanged football-only PMF, frozen calibration, thresholds, and target-excluded same-line consensus; r18 and r11 alter no healthy grade byte. Every evaluated tuple records provider, target sportsbook, exact line, price, market-selection provenance, observation time, independent and calibrated probabilities, blended decision probability, fair probability, edge, EV, threshold release, and lock state. A provider-labeled paired alternate may be evaluated only when no qualified main target cohort exists, it is within one Spread point or two Total points of the dominant main market, and the exact paired line has at least one supported target plus two distinct conventional non-target books. Moneyline alternates remain forbidden. Quote completeness is market-scoped: one unavailable market cannot suppress coherent siblings. A quote timestamped after its evaluation is a structured market-scoped unavailability reason instead of an exception that can unwind the game; artifact and T-60 invariant violations still fail closed. Moneyline and Total retain qualified Best Angle lanes; Spread is capped at Lean. Missing identity, price, consensus, PMF, or T-60 health becomes an explicit public No Play without a fabricated tuple. There is no quota, stake sizing, or reader override.
...
- Current authoritative launch replay is eight games / 24 markets: **2 Best Angles / 4 Leans / 6 Watchlists / 11 evaluated No Plays / 1 Moneyline without a complete grade tuple**. Relative to the pre-repair immutable wave, SJSU-USC Total moves from unavailable to Best Angle and Spread from unavailable to evaluated No Play; there are no demotions and no changes to the other 21 grade tuples. Fresh strict event evidence recovers complete Spread/Total markets across seven SharpAPI books. The 2026-08-28T14:07 ET paid-source audit also finds a real one-sided BetMGM SJSU Moneyline at +6600 and Sportzino at +2000, plus separately labeled Playbook consensus USC -50000 / SJSU +1825. BetMGM +6600 is rejected as a display outlier against the cross-source center; the reader uses the corroborated Sportzino +2000 as one-sided sportsbook context, keeps the primary USC prediction live, and assigns No Play on the Bet-grade axis without synthesizing USC odds or a no-vig probability. r18 changes zero grades: its primary current team scores span 11.50-50.57 (SD 9.71), margin SD is 12.96, and total SD is 4.98. The exact SJSU-USC anchor produces SJSU 11.5-USC 50.6, USC 99.5%, reachable 12-51. Hawaii-Stanford is corrected from the football-only Hawaii 27.2-19.4 / 67.3% baseline to the primary Stanford 26.4-22.4 / 60.5% forecast. Virginia-NC State is primary UVA 27.8-23.8 / 60.6%, which explains why UVA -177 can be a price No Play while UVA -4 -109 is a Lean. Exact-price probabilities and grades remain separately labeled.
- Weekly continuity: `cfb_weekly_window_2026_08_25_r1` selects the Eastern Thursday-through-Monday window anchored by each Tuesday, asks BALLDONTLIE for the bounded UTC-safe date range, then includes every provider-scheduled game where at least one team is FBS. There is no artifact game-ID allowlist. Collection cadence, opening completeness, release refresh, and member completeness are scoped to those current-window IDs; prior-week evidence remains immutable but cannot inflate the current slate. Synthetic Week 0-to-Week 1 and later-season rollover tests prove that new provider IDs are forecast and rendered without code or artifact changes.
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_28_r3_display_quote_coverage`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r4_display_quote_coverage`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r7_display_quote_coverage` / `cfb_forward_evidence_collector_2026_08_28_r10_display_quote_coverage` / `cfb_v1_member_release_2026_08_28_r10_representative_market_quotes` / `cfb_forward_evidence_writer_2026_08_28_r12_display_quote_coverage`. The append-only table schema is unchanged. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, unlocked, and T-60 evidence; no parallel timer or endpoint was added. It consumes every named-book observation returned by BALLDONTLIE for display coverage and the strict SharpAPI exact-event fallback, while only the established six supported target books and qualified comparison cohort can grade. Complete pairs stay in `currentBooks`; verified one-sided main offers stay in `displayBooks` and can appear only as labeled sportsbook context after a representative-market filter rejects Moneyline decimal-odds outliers and Spread/Total line outliers against the cross-book/Playbook center. Before its sole all-game append, writer r12 applies the same shared score/winner, representative-score, PMF, market-count, exact quote, side, EV, actionable-value, and push-aware Moneyline/Spread containment assertion used by NFL. A valid exact-price grade difference is retained and the member explanation states the exact price/EV reason; the gate does not force grades to match. Strict price pagination remains forward-only and bounded; paired-alternate admission is market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. The 2026-08-28 read-only audit returned 11 complete DraftKings Friday events but 0/8 Saturday matches, so the current reader honestly shows pending rather than fabricating coverage. Circa rows may render as Sharp Book Splits after exact matching; DraftKings rows retain `public_recreational` semantics and are never relabeled as sharp, while Playbook remains separately labeled public consensus. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
- Official tracking: `cfb_official_tracking_record_2026_08_26_r2_market_scoped_t60` begins forward-only on the 2026-08-29 ET slate. Each coherent exact-price market frozen at T-60 no more than 20 minutes late can enter `prediction_records`; an internally held/unavailable sibling cannot block it. Zero coherent decisions and global health failures remain ineligible, duplicate/unsupported markets fail closed, retry keys are market-scoped and idempotent, and unlocked grades are never counted. No historical backfill is authorized. Evidence: `docs/model-audits/2026-08-26-football-market-scoped-t60-predeclaration.md`.
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_28_r13_bounded_context_quote_capture` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` keep the mature MLB Daily Edge shell: weekly slate cards, selected-edge interaction, shared three-column hierarchy, responsive/mobile behavior, exact price/movement panels, common grade ladder, and expandable football evidence. The existing Quick Read shows the primary r18 expected score/winner/reachable final and a compact football-only baseline; it adds no football-only dashboard or top-level section. Its market prediction is always model-owned. Bet grade, exact target price, fair probability, EV, thresholds, and lock remain separate. A sub-50% underdog Moneyline value is explicitly labeled as value at price rather than the predicted winner, and Moneyline/Spread grade differences state both exact prices and EVs. Missing complete price evidence is member-facing No Play, never a Held prediction; the internal recovery flag remains machine-visible until the writer repairs coverage. A provider observation stamped no more than five seconds after the writer's run-start timestamp may remain visible only as non-graded, one-sided context from the same bounded response; it cannot become a selected price, fair pair, probability, grade, or tracking tuple, and later rows still fail closed. Up to five prioritized football drivers remain visible and every additional supplied row stays reachable in the existing disclosure. The fixture atomically selects a complete r7/r10/r11 wave and has bounded whole-wave fallbacks through r6/r8/r11, r5/r7/r10, r4/r6/r10, r3/r5/r9, r2/r4/r7, and r1/r2/r5; it never mixes releases. `CFB_DAILY_EDGE_ENABLED=true` and `CFB_FORWARD_EVIDENCE_ENABLED=true` retain their gates. Immediate rollback restores the complete r6/r8/r11 wave while preserving the r18 outcome contract; older evidence remains immutable and release-scoped.
- Reader-axis patch: shared presentation release `daily_edge_member_presentation_2026_08_28_r10_football_prediction_bet_axis` keeps the prediction tab label and probability attached to the primary r18 `marketPrediction`, while the Quick Read exact-price panel keeps the evaluated side, book, price, selected-side probability, and Bet Grade attached to `pick`. When the primary outcome head and qualified price head disagree, both sides are named explicitly; the UI can no longer show the model side with the opposing side's price or grade probability. This changes zero PMFs, decisions, grades, stakes, locks, or tracking rows.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbV1WeeklyForecast.ts`, `lib/services/football/cfbMarketInformedOutcome.ts`, `lib/services/football/footballCrossMarketCoherence.ts`, `lib/services/football/cfbWeeklyWindow.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbSharpApiOdds.ts`, `lib/services/football/cfbSharpApiSplits.ts`, `lib/services/football/footballMarketScopedTracking.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the versioned score/weekly/grade/market-residual JSON artifacts. Evidence: `docs/model-audits/2026-08-28-football-cross-market-coherence-predeclaration.md`, `docs/model-audits/2026-08-28-football-cross-market-coherence-r19.md`, `docs/model-audits/2026-08-25-cfb-generalized-weekly-engine-predeclaration.md`, `docs/model-audits/2026-08-25-cfb-v1-composite-grade-release.md`, `docs/model-audits/2026-08-26-cfb-sharpapi-price-fallback-r7.md`, `docs/model-audits/2026-08-26-football-market-scoped-t60-predeclaration.md`, `docs/model-audits/2026-08-27-cfb-pmf-side-guard-pagination-r16.md`, `docs/model-audits/2026-08-28-cfb-market-informed-outcome-r18-predeclaration.md`, and `docs/model-audits/2026-08-28-cfb-market-informed-outcome-r18.md`.
...
Shared member presentation release: `daily_edge_member_presentation_2026_08_28_r10_football_prediction_bet_axis`.
