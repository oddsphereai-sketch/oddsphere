...
- Current authoritative replay: latest 16-game/48-market wave captured 2026-08-25T15:36:33.686Z produces **3 Best Angles / 12 Leans / 5 Watchlists / 28 No Plays**. Moneyline: 3 / 5 / 2 / 6. Spread: 0 / 3 / 3 / 10. Total: 0 / 4 / 0 / 12. Versus the prior member behavior this is 13 promotions, 0 demotions, and +7 actionable markets. Best Angles are CHI ML -142 DraftKings, MIN ML -108 BetRivers, and DAL ML -146 FanDuel. New Spread Leans are CAR +2.5 +102 FanDuel, HOU +1.5 -112 BetRivers, and WSH +4.5 -110 Caesars. New Total Leans are NE-SEA Over 44.5 -105 Fanatics, BAL-IND Under 48.5 -110 BetMGM, BUF-HOU Over 44.5 -109 Caesars, and DEN-KC Under 42.5 -107 Caesars.
- Writer/releases: evidence schema / collector remain `nfl_forward_evidence_snapshot_2026_08_23_r3_member` / `nfl_forward_evidence_collector_2026_08_23_r3_member`; the single authoritative leased writer is `nfl_forward_evidence_writer_2026_08_27_r10_compact_member_snapshot`. It remains the only prediction writer under `prediction_pipeline:nfl`; no parallel endpoint or reader-side override exists. Writer r10 preserves r9 market-scoped T-60 behavior and additionally packages the exact release-keyed member fixture into `nfl_forward_member_snapshot_2026_08_27_r1_compact` after the raw append, or refreshes that compact row on a natural cadence-not-due cycle without a provider call. The indexed row is a read optimization only: append-only raw evidence remains authoritative, a release/checksum guard rejects mixed payloads, and the reader falls back to the raw builder if the compact row is absent or invalid. When an upcoming pre-T60 game's latest row does not contain exactly three current r9 decisions under the current r6 member release, the next natural scheduled cycle still performs one provider-backed unlocked refresh for the full slate instead of waiting for the ordinary six-hour cadence or relabeling old quotes. T-60 work retains priority, and completed/no-unlocked slates do not generate release-transition provider calls. Strict NFL SharpAPI rows remain distinct from Playbook public consensus.
...
- Current authoritative launch replay is eight games / 24 markets: **2 Best Angles / 4 Leans / 6 Watchlists / 11 evaluated No Plays / 1 unavailable Moneyline**. Relative to the pre-repair immutable wave, SJSU-USC Total moves from unavailable to Best Angle and Spread from unavailable to evaluated No Play; there are no demotions and no changes to the other 21 grade tuples. Fresh strict event evidence recovers BetMGM SJSU +39 -108 / USC -39 -110 and Over 60.5 -108 / Under 60.5 -110; the exact grade sides are SJSU +39 No Play and Under 60.5 Best Angle. Moneyline remains unavailable because no coherent two-sided target/consensus tuple exists and cannot suppress Spread or Total. r18 changes zero grades: its primary current team scores span 11.50-50.57 (SD 9.71), margin SD is 12.96, and total SD is 4.98. The exact SJSU-USC anchor produces SJSU 11.5-USC 50.6, USC 99.5%, reachable 12-51. Hawaii-Stanford is corrected from the football-only Hawaii 27.2-19.4 / 67.3% baseline to the primary Stanford 26.4-22.4 / 60.5% forecast. Virginia-NC State is primary UVA 27.8-23.8 / 60.6%, which explains why UVA -177 can be a price No Play while UVA -4 -109 is a Lean. Exact-price probabilities and grades remain separately labeled.
...
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_26_r2_opened_at`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r3_exact_paired_market_evidence`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r5_two_axis_outcome_sharp_splits` / `cfb_forward_evidence_collector_2026_08_28_r8_two_axis_outcome_sharp_splits` / `cfb_v1_member_release_2026_08_28_r7_two_axis_outcome_sharp_splits` / `cfb_forward_evidence_writer_2026_08_28_r9_two_axis_outcome_sharp_splits`. The append-only table schema is unchanged. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, unlocked, and T-60 evidence; no parallel timer, provider path, or reader-side recomputation was added. Strict price pagination remains forward-only and bounded; paired-alternate admission is market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. The 2026-08-28 read-only audit returned 11 complete DraftKings Friday events but 0/8 Saturday matches, so the current reader honestly shows pending rather than fabricating coverage. Circa rows may render as Sharp Book Splits after exact matching; DraftKings rows retain `public_recreational` semantics and are never relabeled as sharp, while Playbook remains separately labeled public consensus. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
...
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_28_r9_two_axis_outcome_sharp_splits` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` keep the mature MLB Daily Edge shell: weekly slate cards, selected-edge interaction, shared three-column hierarchy, responsive/mobile behavior, exact price/movement panels, common grade ladder, and expandable football evidence. The existing Quick Read shows the primary r18 expected score/winner/reachable final and a compact football-only baseline; it adds no football-only dashboard or top-level section. Bet grade, price, fair probability, EV, thresholds, and lock remain separate. Up to five prioritized football drivers remain visible and every additional supplied row stays reachable in the existing disclosure. The fixture atomically selects a complete r5/r7/r10 wave and has bounded whole-wave fallbacks through r4/r6/r10, r3/r5/r9, r2/r4/r7, and r1/r2/r5; it never mixes releases. Internal exceptions preserve primary and secondary forecasts while withholding only the incomplete exact-price tuple. `CFB_DAILY_EDGE_ENABLED=true` and `CFB_FORWARD_EVIDENCE_ENABLED=true` retain their gates. Immediate rollback is exact decision r10 / tuple r4 / evidence r4 / member r6 / writer r8 / fixture r8 / Sharp price r3 with the single-axis reader fallback; older r3/r5/r9, r2/r4/r7, and r1/r2/r5 evidence remains immutable and release-scoped.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbV1WeeklyForecast.ts`, `lib/services/football/cfbMarketInformedOutcome.ts`, `lib/services/football/cfbWeeklyWindow.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbSharpApiOdds.ts`, `lib/services/football/cfbSharpApiSplits.ts`, `lib/services/football/footballMarketScopedTracking.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the versioned score/weekly/grade/market-residual JSON artifacts. Evidence: `docs/model-audits/2026-08-25-cfb-generalized-weekly-engine-predeclaration.md`, `docs/model-audits/2026-08-25-cfb-v1-composite-grade-release.md`, `docs/model-audits/2026-08-26-cfb-sharpapi-price-fallback-r7.md`, `docs/model-audits/2026-08-26-football-market-scoped-t60-predeclaration.md`, `docs/model-audits/2026-08-27-cfb-pmf-side-guard-pagination-r16.md`, `docs/model-audits/2026-08-28-cfb-market-informed-outcome-r18-predeclaration.md`, and `docs/model-audits/2026-08-28-cfb-market-informed-outcome-r18.md`.
