...
### Complete prediction-accuracy denominators (tracking r14 / writer r43)
...
- Writer / tracking releases are `cfb_forward_evidence_writer_2026_09_04_r43_complete_tracking_denominators` / `cfb_official_tracking_record_2026_09_04_r14_complete_prediction_denominators`. The existing market-scoped idempotency key naturally inserts only missing `(game, market)` rows from retained immutable T-60 evidence; it never overwrites an existing record or invents a price. The September 3 cohort changes from 6 Moneyline / 10 Spread / 9 Total rows to 10/10/10 when every held outlook is available. Evidence and rollback: `docs/model-audits/2026-09-04-cfb-complete-tracking-denominators.md`.
