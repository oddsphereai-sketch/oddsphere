...
- Current authoritative replay covers 38 games / 114 markets from the immutable 2026-08-28T20:09 ET wave. Before repair, 23 markets had exact tuples: **2 Best Angles / 3 Leans / 9 Watchlists / 9 evaluated No Plays**, with 91 unavailable. The candidate evaluates 26 exact tuples: **3 Best Angles / 3 Leans / 9 Watchlists / 11 evaluated No Plays**, with 88 unavailable—three market-availability promotions, zero demotions, and no change to any pre-existing tuple or grade. SJSU-USC recovers SJSU +38.5 FanDuel -104 as No Play and Under 61.5 FanDuel -110 as Best Angle; Moneyline remains unavailable because no coherent two-sided target quote exists. MTST-UTU also recovers Under 56.5 BetMGM -108 as evaluated No Play. All 38 games pass strict cross-market coherence. UC Davis-Portland State changes only its internally contradictory forecast from UCD 26.49-PRST 26.66 / UCD 50.55% / reachable UCD 27-26 to UCD 26.59-PRST 26.55 / UCD 51.13% / reachable UCD 27-26; its three unavailable markets remain unchanged. Hawaii-Stanford remains primary Stanford 26.4-22.4 / 60.5% with the football-only Hawaii baseline explicitly secondary; Virginia-NC State remains primary UVA 27.8-23.8 / 60.6%. Strict Sharp splits currently match 2/38 games and unmatched rows never render.
- Weekly continuity: `cfb_weekly_window_2026_08_28_r2_model_covered_division_i` selects the Eastern Thursday-through-Monday window anchored by each Tuesday and asks BALLDONTLIE for the bounded UTC-safe date range. The writer now includes every provider-scheduled NCAAF matchup whose two team identities resolve unambiguously to the qualified 256-team artifact, including model-covered FCS-only games. Unknown/ambiguous team identities are excluded instead of neutral-imputed into the betting board. A game first discovered after kickoff is not backfilled, while a game with existing immutable evidence remains inside normal lifecycle handling. There is no artifact game-ID allowlist. Collection cadence, opening completeness, release refresh, and member completeness are scoped to those current-window IDs; prior-week evidence remains immutable but cannot inflate the current slate.
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_28_r3_display_quote_coverage`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_28_r8_canonical_event_discovery`; strict split client `cfb_sharpapi_splits_2026_08_28_r1_strict_identity`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_28_r9_canonical_price_coverage` / `cfb_forward_evidence_collector_2026_08_28_r15_canonical_event_discovery` / `cfb_v1_member_release_2026_08_28_r16_canonical_price_coverage` / `cfb_forward_evidence_writer_2026_08_28_r19_canonical_event_discovery`. Member fixture release is `cfb_v1_member_fixture_2026_08_28_r20_canonical_price_coverage`. The append-only table schema is unchanged, and the complete r8 wave remains the atomic fallback until a complete r9 wave exists. The one scheduled writer remains the sole owner of the `prediction_pipeline:cfb` lease and appends Opening, unlocked, and T-60 evidence; no parallel timer or endpoint was added. It consumes every named-book observation returned by BALLDONTLIE for display coverage and the strict SharpAPI exact-event fallback, while only the established six supported target books and qualified comparison cohort can grade. Complete pairs stay in `currentBooks`; verified one-sided main offers stay in `displayBooks` and can appear only as sportsbook context after a representative-market filter rejects Moneyline decimal-odds outliers and Spread/Total line outliers against the cross-book/Playbook center. Before its sole all-game append, writer r19 applies strict shared coherence release `football_cross_market_coherence_2026_08_28_r3_strict_directional_pmf`; any expected-score/winner-probability direction cross is fatal. PMF identity, representative winner, market count, exact quote, side, EV, actionable value, and push-aware Moneyline/Spread containment remain fatal. A valid exact-price grade difference is retained and the member explanation states the exact price/EV reason; the gate does not force grades to match. Sharp r8 first discovers canonical NCAAF events once per unique Eastern/UTC slate date, then requires one exact home/away/time match before requesting `market=main` odds for that event ID. It no longer constructs or probes guessed event bucket IDs. Strict price pagination remains forward-only and bounded; zero event matches remain unavailable, while ambiguity or canonical-event reuse fails before the all-game append. The expanded Division I slate retains a 192-request all-run hard cap, and repeated, empty, conflicting, non-advancing, over-cap, or malformed pages still fail before append. Paired-alternate admission remains market-scoped and does not let a missing Moneyline suppress Spread/Total. The same writer makes one bounded league-level NCAAF split request for the slate and attaches rows only after exact league/team/date identity. A complete Circa row has first priority; until it is published, a complete DraftKings row occupies the established Sharp Book Splits member card and is replaced automatically by Circa. Playbook remains the separate Public Consensus card. A split request failure or upstream-not-yet-published event is a context warning, never a whole-game hold. The immutable capture/evaluation timestamp is the latest real provider observation included in each payload, so bounded responses received seconds after run start remain valid without tolerating any quote after final capture. Timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
