...
### Complete week-ahead availability recovery (r64)

- Active weekly window / sole writer / member fixture / compact snapshot: `cfb_weekly_window_2026_09_07_r4_overlapping_week_ahead` / `cfb_forward_evidence_writer_2026_09_13_r58_latest_game_rows` / `cfb_v1_member_fixture_2026_09_13_r52_live_prediction_visibility` / `cfb_forward_member_snapshot_2026_09_13_r10_live_prediction_visibility`. The writer selects one checksum-verified latest current-release payload per game and capture stage instead of reloading 12,024 superseded and repeated large JSON payloads; the lightweight identity/date trail remains complete for unchanged prior-results inputs. The September 17–21 provider window contains 101 model-covered games, 100 of which currently request supplemental named-book enrichment. It rotates `cfb_sharpapi_named_book_fallback_2026_09_13_r12_bounded_writer_deadline` through a deterministic 24-game per-run and 40-second whole-attempt budget while every model-covered game continues through the sole append and compact-snapshot path. A missing exact-price tuple stays market-level No Play and cannot erase the live model projection or label the entire game held.
- NFL and CFB availability recovery changes no score model, PMF, calibration, side, probability, grade rule, stake, T-60 lock, tracking, or settlement behavior. Same-input promotion/demotion and actionable-board impact is zero; only scheduled-game visibility and writer completion change. Evidence and rollback: `docs/model-audits/2026-09-13-football-daily-edge-availability-predeclaration.md`.
