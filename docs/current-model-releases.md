...
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_25_r3_same_book_history` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` scope the normal Daily Edge slate/reader to the active weekly window, keep each game visible through its Eastern date, and remove prior-date games without touching stored lock/tracking history. Fixture r3 keeps the latest row authoritative for forecasts, decisions, grades, splits, and lock state while reconstructing compact, chronological first/material-change/current movement trails from immutable rows for the exact evaluated sportsbook. `CFB_DAILY_EDGE_ENABLED=true` selects the normal Daily Edge slate cards and selected-edge reader. `CFB_FORWARD_EVIDENCE_ENABLED=true` enables the sole writer. A real per-market health problem renders Held within the normal product. Disabling either gate stops new publication or hides the sport without deleting immutable evidence or tracking records.
