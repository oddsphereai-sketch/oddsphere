...
- NFL tracking record / sole writer are `nfl_official_tracking_record_2026_09_04_r6_complete_prediction_denominators` / `nfl_forward_evidence_writer_2026_09_13_r23_game_scoped_odds_gaps`. An eligible T-60 payload emits all three immutable forecast markets. A market without a coherent exact-price tuple is stored as a side-bearing Held No Play with null price/market probability/edge/EV, so it counts for prediction accuracy but cannot enter Best Angle, Lean, stake, or ROI. True whole-game health failures remain ineligible. The regular slate adapter `balldontlie_nfl_regular_slate_2026_09_13_r3_game_scoped_odds_gaps` retains the complete verified schedule when current odds have disappeared for completed games; only a due game's missing tuple is isolated by the writer. Compact snapshot `nfl_forward_member_snapshot_2026_09_13_r8_game_scoped_odds_gaps` publishes the resulting complete current-release wave.
...
- Active weekly window / sole writer / member fixture / compact snapshot: `cfb_weekly_window_2026_09_07_r4_overlapping_week_ahead` / `cfb_forward_evidence_writer_2026_09_13_r57_bounded_week_ahead_recovery` / `cfb_v1_member_fixture_2026_09_13_r52_live_prediction_visibility` / `cfb_forward_member_snapshot_2026_09_13_r10_live_prediction_visibility`. The September 17–21 provider window contains 101 model-covered games, 100 of which currently request supplemental named-book enrichment. The writer now rotates `cfb_sharpapi_named_book_fallback_2026_09_13_r12_bounded_writer_deadline` through a deterministic 24-game per-run and 40-second whole-attempt budget while every model-covered game continues through the sole append and compact-snapshot path. A missing exact-price tuple stays market-level No Play and cannot erase the live model projection or label the entire game held.
