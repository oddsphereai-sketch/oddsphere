...
- Writer and provider boundary: BALLDONTLIE slate `balldontlie_ncaaf_slate_2026_08_26_r2_opened_at`; Sharp price fallback `cfb_sharpapi_named_book_fallback_2026_08_27_r2_bounded_pagination`; evidence schema / collector / member / writer `cfb_forward_evidence_snapshot_2026_08_27_r3_pmf_side_guard` / `cfb_forward_evidence_collector_2026_08_27_r6_pagination_side_guard` / `cfb_v1_member_release_2026_08_27_r5_pmf_side_guard` / `cfb_forward_evidence_writer_2026_08_27_r7_pagination_side_guard`. The append-only table schema is unchanged. One scheduled writer owns the `prediction_pipeline:cfb` lease and captures immutable Opening/T-60 evidence plus bounded unlocked refreshes. BALLDONTLIE remains authoritative for schedule and primary prices; planned games without complete same-line coverage receive bounded, strict exact-event SharpAPI named-book reads before the same sole append. Exact-event pagination follows at most four forward-only cursor pages and remains inside the existing 96-request whole-run cap; malformed, repeated, or over-cap cursors fail closed. Only supported US books may be evaluated targets; other trusted books are consensus-only. Writer r7 counts the exact coherent market set for T-60 tracking rather than a three-market game multiplier. QB collection, fail-closed feature health, portable weekly forecasting, and provider budgets remain unchanged. Same-PMF outlooks remain forecast context when an exact-price tuple is unavailable. Playbook consensus is separate. SharpAPI NCAAF splits, timestamped injuries, confirmed depth/starters, and venue weather remain unavailable and are never fabricated.
...
- Reader/rollback: member fixture `cfb_v1_member_fixture_2026_08_27_r6_pmf_side_guard` and `daily_edge_weekly_reader_lifecycle_2026_08_25_r3_cfb` scope the normal Daily Edge slate/reader to the active weekly window and keep the latest complete release authoritative. The common route, DTO types, reader shell, sport switching, modal behavior, and presentation components remain shared with mature MLB behavior. During transition it serves a complete legacy/prior wave until the sole writer atomically appends a complete r3/r5/r9 wave; it never mixes releases. Compact movement remains chronological and same-book. Every internal exception preserves the independent outcome forecast and score context; only its evaluated sportsbook side/price, fair probability, edge, EV, actionability, grade price, and tracking proposal are absent. The shared presentation boundary renders that Bet grade as a reasoned No Play while internal recovery remains active. `CFB_DAILY_EDGE_ENABLED=true` and `CFB_FORWARD_EVIDENCE_ENABLED=true` retain their existing gates. Rollback is decision r7 / tuple r2 / evidence r2 / member r4 / writer r6 / fixture r5 / Sharp fallback r1; immutable evidence and tracking remain release-scoped.
- Runtime authority: `lib/services/football/cfbV1Decision.ts`, `lib/services/football/cfbV1WeeklyForecast.ts`, `lib/services/football/cfbWeeklyWindow.ts`, `lib/services/football/cfbForwardEvidenceWriter.ts`, `lib/services/football/cfbSharpApiOdds.ts`, `lib/services/football/footballMarketScopedTracking.ts`, `lib/services/football/cfbOfficialTrackingRecord.ts`, `lib/services/football/cfbMemberFixture.ts`, and the three versioned JSON model artifacts. Evidence: `docs/model-audits/2026-08-25-cfb-generalized-weekly-engine-predeclaration.md`, `docs/model-audits/2026-08-25-cfb-v1-composite-grade-release.md`, `docs/model-audits/2026-08-26-cfb-sharpapi-price-fallback-r7.md`, `docs/model-audits/2026-08-26-football-market-scoped-t60-predeclaration.md`, and `docs/model-audits/2026-08-27-cfb-pmf-side-guard-pagination-r16.md`.
