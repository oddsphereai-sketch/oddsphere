...
- Writer/releases: evidence schema / collector are `nfl_forward_evidence_snapshot_2026_08_31_r4_weekly_market_injury` / `nfl_forward_evidence_collector_2026_08_31_r4_weekly_market_injury`; the single authoritative leased writer is `nfl_forward_evidence_writer_2026_08_31_r16_market_split_injury`, and the compact member snapshot is `nfl_forward_member_snapshot_2026_08_31_r3_market_split_injury`. The store reads r4/r3/r2/r1 with bounded 1,000-row pagination and a 5,000-row per-release cap. Writer r16 contains a malformed game at game scope, preserves one `prediction_pipeline:nfl` lease, one append path and immutable T-60 priority, and retains strict named-book split provenance. A fragmented exact-line board may use one target-excluded same-line comparator to keep a complete conservative prediction tuple, but it cannot receive an actionable grade unless at least two target-excluded same-line comparators exist.
...
- Reader/rollback: member fixture `nfl_weekly_member_fixture_2026_08_31_r12_market_split_injury` consumes the release-keyed weekly evidence, preserves the stored opening price during an exact-price health exception, shows writer-owned movement and real availability/health reasons instead of release identifiers, and keeps the existing grade/copy product. `NFL_WEEK_ONE_EVIDENCE_BOARD_ENABLED=true` selects the evidence board; `NFL_DAILY_EDGE_ENABLED` remains the visibility rollback.
...
- Active releases: model `nfl_player_props_distribution_model_2026_08_25_r2_shared_context`; calibration `nfl_player_props_distribution_calibration_2026_08_25_r2_shared_context`; decision `nfl_player_props_decision_2026_08_31_r3_monotonic_divergence`; runtime `nfl_player_props_runtime_2026_08_31_r3_monotonic_divergence`; board DTO `nfl_player_props_board_2026_08_31_r6_monotonic_divergence`; member snapshot `nfl_player_props_member_2026_08_31_r8_monotonic_divergence`; writer `nfl_player_props_writer_2026_08_31_r9_opening_closing_order`; tracking `nfl_player_props_tracking_2026_08_31_r5_closing_before_settlement`; settlement `nfl_player_props_settlement_2026_08_25_r3_bounded_finality`. The frozen runtime artifact remains checksum provenance at r2; the behavior-bearing scorer and decision identifiers are r3.
...
- Exact-price policy: every graded outcome requires a complete target-book line/side price and at least one separate same-line book for a target-excluded market benchmark. Game-level split feeds are not a props dependency and their absence never Holds a prop; the sport-owned market evidence is player projection/participation and injury context plus multi-book price and same-book opening/current/closing movement. Stale, incomplete, missing-feature, and no-independent-benchmark outcomes are unavailable diagnostics, not Held member plays. Held is reserved for genuine timestamped role/player-identity ambiguity.
