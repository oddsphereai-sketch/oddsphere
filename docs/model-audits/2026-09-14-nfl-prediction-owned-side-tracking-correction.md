...
Historical corrections are append-only. Each correction has a deterministic ID, identifies the superseded original, preserves the original evidence snapshot with explicit correction metadata, and uses the published forecast probability plus the real contemporaneous prediction-side price. Original rows are never changed or deleted. Public aggregates and the admin winner scorecard remove a superseded original only when its explicit correction is present.
