...
console.log("\n━━━ P7-Commit-B — FI v2 play_grade persistence ━━━");
{
  ...
  check("FI conf=58 legacy fallback → best_angle stays false", fi.best_angle === false);
  check("FI conf=58 → no_bet stays false", fi.no_bet === false);
}
{
  // Current FI v2 writer grade is authoritative: Best Angle must persist both
  // play_grade and the boolean used by locked Daily Edge cards/tracking.
  const fiBestAngle = {
    ...basePrediction,
    predicted_nrfi: true,
    nrfi_confidence: 56,
...
}
{
  ...
  check("FI conf=65 legacy fallback → best_angle stays false", fi.best_angle === false);
}
{
