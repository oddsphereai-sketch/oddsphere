...
/**
 * Push 3B-5 — exported cycle so the feature-coverage-refresh cron can
 * delegate BDL player backfill. Same classification rules as the CLI
 * path; never auto-creates rows in DRY-RUN.
 */
export type BdlBackfillCounts = {
  already_linked: number;
  link: number;
  create: number;
  skip_ambiguous: number;
  skip_missing_team: number;
  skip_missing_name: number;
  skip_team_unmapped: number;
};
...
type Plan = { bdl_id: number; full_name: string; bdl_team_id: number | null; bdl_team_abbr: string | null; db_team_id: number | null; action: Action; db_player_id?: number | null; note?: string; raw: BdlPlayer };

function emptyCounts(): BdlBackfillCounts {
  return { already_linked: 0, link: 0, create: 0, skip_ambiguous: 0, skip_missing_team: 0, skip_missing_name: 0, skip_team_unmapped: 0 };
}
function countPlans(plans: Plan[]): BdlBackfillCounts {
  const c = emptyCounts();
  for (const p of plans) c[p.action]++;
  return c;
}
async function classifyBdlPlayers(playerByBdlId: Map<number, BdlPlayer>, bdlPlayerMap: Awaited<ReturnType<typeof loadPlayerBdlIdMap>>, teamIdByExternal: Map<number, number>, sport: Sport): Promise<Plan[]> {
  const plans: Plan[] = [];
  for (const [bdlId, p] of playerByBdlId) {
    if (bdlPlayerMap.has(bdlId)) { plans.push({ bdl_id: bdlId, full_name: p.full_name, bdl_team_id: p.team?.id ?? null, bdl_team_abbr: p.team?.abbreviation ?? null, db_team_id: null, action: "already_linked", raw: p }); continue; }
    if (!p.full_name) { plans.push({ bdl_id: bdlId, full_name: "(unknown)", bdl_team_id: p.team?.id ?? null, bdl_team_abbr: p.team?.abbreviation ?? null, db_team_id: null, action: "skip_missing_name", raw: p }); continue; }
    if (!p.team || typeof p.team.id !== "number") { plans.push({ bdl_id: bdlId, full_name: p.full_name, bdl_team_id: null, bdl_team_abbr: null, db_team_id: null, action: "skip_missing_team", raw: p }); continue; }
    const dbTeamId = teamIdByExternal.get(p.team.id);
    if (dbTeamId === undefined) { plans.push({ bdl_id: bdlId, full_name: p.full_name, bdl_team_id: p.team.id, bdl_team_abbr: p.team.abbreviation, db_team_id: null, action: "skip_team_unmapped", raw: p }); continue; }
    const { data: matches } = await supabase.from("players").select("id, full_name, team_id, active, provider_ids").eq("sport", sport).eq("team_id", dbTeamId).ilike("full_name", p.full_name);
