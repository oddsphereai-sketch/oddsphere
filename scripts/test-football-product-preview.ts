...
assert.match(pageSource, /loadNflRegularPipelinePreseasonSlate/);
assert.match(pageSource, /No fallback or fabricated slate is shown/);
assert.match(pageSource, /activePreviewSports=\{\["nfl"\]\}/);
assert.match(pageSource, /preseason never tracked/);
assert.match(pageSource, /one verified board/);
assert.match(pageSource, /football-preview-real-nfl-single-stored-slate-v9/);
assert.match(pageSource, /loadNflPreseasonLocalSlate/);
assert.match(pageSource, /phaseComparisonSlate/);
assert.match(pageSource, /48 predictions|games\.length \* 3/);
assert.match(pageSource, /Opening trails/);
assert.match(pageSource, /resolveNflPreviewWeek\(2\)/);
assert.doesNotMatch(pageSource, /fetchBalldontlieNflPreviewSlate/);
assert.doesNotMatch(pageSource, /query\.phase|query\.week/);
assert.match(pageSource, /revalidate: 5 \* 60/);
...
assert.match(readerSource, /Grade counts cover all/);
...
console.log("Football product preview: single checksum-matched stored board, real preseason evidence, and tracking exclusion passed");
