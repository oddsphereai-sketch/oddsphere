...
assert.equal(MLB_PROPS_MODEL_RELEASE_ID, "mlb_props_2026_08_19_r36");
...
assert.ok(liveBoard.includes("projectBatterHomeRunsPortfolio"));
assert.ok(liveBoard.includes("HOME_RUN_PA_PORTFOLIO_READ"));
assert.ok(liveBoard.includes("applyValidatedHomeRunPortfolioPromotions"));
assert.ok(liveBoard.includes("VALIDATED_HOME_RUN_PA_PORTFOLIO_LEAN"));
assert.ok(liveBoard.includes("VALIDATED_HOME_RUN_MEDIUM_PRICE_COMPLEMENT_LEAN"));
assert.ok(liveBoard.includes("VALIDATED_RBI_VALUE_PORTFOLIO_LEAN"));
