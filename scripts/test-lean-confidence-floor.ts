/**
 * Conviction/Value play-grade gate regression (2026-06-15). Replaces the earlier
 * R1 confidence-floor (which emptied the board). Verifies the three evidence-gated,
 * environment-checked demotions and that non-Lean grades + clean Leans are untouched:
 *   1. EV<0 (coherence, arithmetic)         2. low-conviction ML favorite (run-gap<0.5)
 *   3. extreme-low total line (<8, pitcher's duel — no model edge)
 */
import { applyPlayGradeGate, GATE_LOW_CONVICTION_RUNGAP, GATE_LOW_TOTAL_LINE } from "../lib/services/predictionRecordService";
...
// 1. EV gate (coherence): fav -150 → breakeven 60%; model 54% = EV<0 → demote
...
check("lean dog +120 @ 0.50 (EV>0) stays lean", ml(0.50, 120, 0.8) === "lean");

// 2. low-conviction favorite (run-gap < 0.5)
