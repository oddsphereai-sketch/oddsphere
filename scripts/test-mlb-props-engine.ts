...
async function main() {
  ...
  check("home-run model uses the validated diversified PA portfolio", liveBoardSource.includes("HITTER_WATCHLIST_ONLY_MARKETS") && liveBoardSource.includes('"batter_home_runs"') && liveBoardSource.includes("applyValidatedHomeRunPortfolioPromotions") && liveBoardSource.includes("VALIDATED_HOME_RUN_PA_PORTFOLIO_LEAN"));
