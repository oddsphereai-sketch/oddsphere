...
async function main() {
  ...
  check("props snapshot retention is bounded while locked references are preserved", boardSnapshotStoreSource.includes("DEFAULT_MLB_PROPS_SNAPSHOT_RETENTION_PER_SLATE = 12") && boardSnapshotStoreSource.includes("referencedSnapshotIds") && boardSnapshotStoreSource.includes("board_snapshot_id"));
