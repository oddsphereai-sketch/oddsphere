...
async function main() {
  ...
  const propsProductUiSource = readFileSync("app/player-props/components/PlayerPropsProductUi.tsx", "utf8");
  ...
  check("props UI is a researcher rather than a pick feed", propsUiSource.includes("Prop Researcher") && (propsUiSource.includes("Today&apos;s Radar") || propsUiSource.includes("Today’s Radar")) && sharedReaderSource.includes('eyebrow = "Prop Reader"') && !propsUiSource.includes("Best Bets") && !propsUiSource.includes("All Props") && !propsUiSource.includes("VIEW_OPTIONS"));
  ...
  check("compact slate header has one clear contextual purpose", ["PropsSlateHeader", "SlateGameNavigator", "Prices updated", "Today’s games"].every((label) => propsUiSource.includes(label)) && propsProductUiSource.includes("Board status") && !propsUiSource.includes("Top value"));
