...
async function main() {
  ...
  check("player directory opens a focused player workspace", radarSource.includes("PlayerDirectory") && propsUiSource.includes("playerDirectorySummaries") && playerViewSource.includes('data-product-zone="player-workspace"') && playerViewSource.includes("Back to Radar") && playerViewSource.includes(">Read<"));
  ...
  check("market board exposes model-tool comparison columns", ["Player", "Market", "Pick / Line", "Book / Odds", "Edge", "EV", "Coverage", "Model Signal", "Reader"].every((label) => boardSource.includes(`>${label}<`)) && boardSource.includes("projectionLabel") && boardSource.includes('"Recent avg" : "Projection"') && !boardSource.includes(">Reason<"));
  ...
  check("grade helpers use analytical member labels", getPropGradeLabel("BEST_ANGLE") === "Strong Signal" && getPropGradeLabel("LEAN") === "Positive Signal" && getPropGradeLabel("NO_PLAY") === "No Edge" && getPropGradeDescription("BEST_ANGLE").length > 0);
  check("player-prop status colors keep Watchlist and No Play neutral", getPropGradeColor("BEST_ANGLE").border === "#10b981" && getPropGradeColor("LEAN").border === "#38bdf8" && getPropGradeColor("WATCHLIST").border === "#64748b" && getPropGradeColor("NO_PLAY").border === "#4b5563");
  ...
  check("dev preview stays production-disabled", devPreviewSource.includes("process.env.NODE_ENV === \"production\"") && devPreviewSource.includes("notFound()"));
