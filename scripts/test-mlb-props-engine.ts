...
async function main() {
  ...
  check("researcher is the default rendered workflow", propsUiSource.includes('useState<SortKey>("player")') && propsUiSource.includes("<TodayRadar") && propsUiSource.includes("<PlayerDirectory") && propsUiSource.includes("<FullBoardView rows={filteredRows}") && !propsUiSource.includes("TodayAnglesView"));
  check("researcher leads with universal search and quick market controls", researchEntrySource.includes("Search a player, team, market, or sportsbook") && researchEntrySource.includes('id="mlb-props-players"') && researchEntrySource.includes('aria-label="Market filters"') && researchEntrySource.includes("MarketFilterButton"));
  check("researcher exposes model signals as a board filter", researchEntrySource.includes('label="Model signal"') && researchEntrySource.includes("PROP_GRADES.map") && !researchEntrySource.includes("Strongest model edges"));
  ...
  check("market board exposes model-tool comparison columns", ["Player", "Market", "Side / Line", "Book / Odds", "Edge", "EV", "Coverage", "Model Signal", "Reader"].every((label) => boardSource.includes(`>${label}<`)) && boardSource.includes("projectionLabel") && boardSource.includes('"Recent avg" : "Projection"') && !boardSource.includes(">Reason<"));
  ...
  check("best-price board pairs both sides and uses grade-colored signal side", propsUiSource.includes("MarketPairCards") && propsUiSource.includes("pairSignalRow") && propsUiSource.includes("Model side:") && propsUiSource.includes("getPropGradeColor(signal.playGrade)") && propsUiSource.includes("Research only") && propsUiSource.includes("Not offered") && !propsUiSource.includes("Projection side"));
  ...
  check("drawer explains projection-side mismatches in member language", drawerSource.includes("ProjectionIntegrityNotice") && drawerSource.includes("Projection points to") && drawerSource.includes("does not support") && !drawerSource.includes("Blocked projection-side contradiction") && !drawerSource.includes("excluded from positive model signals"));
  ...
  check("member reason and context translators remove operational language", propsUiSource.includes("function memberReason") && propsUiSource.includes('STALE_BDL_ODDS: "Price is updating"') && propsUiSource.includes('FIRST_HR_FIELD_MODEL_NOT_PROMOTED: "Research market only"') && propsUiSource.includes('.replace(/^BDL\\s+/i, "")') && propsUiSource.includes('.replace(/^MLB Stats\\s+/i, "")') && propsUiSource.includes("verified fixture field"));
  ...
  check("hitter market tiers allow volume leans while capping rare events", liveBoardSource.includes("HITTER_LEAN_ELIGIBLE_MARKETS") && liveBoardSource.includes('"batter_hits"') && liveBoardSource.includes('"batter_total_bases"') && liveBoardSource.includes("HITTER_WATCHLIST_ONLY_MARKETS") && liveBoardSource.includes('"batter_home_runs"') && liveBoardSource.includes("RARE_OR_CONTEXT_HEAVY_MARKET_CAPPED"));
  ...
  check("grade helpers use Daily Edge-aligned member labels", getPropGradeLabel("BEST_ANGLE") === "Best Angle" && getPropGradeLabel("LEAN") === "Lean" && getPropGradeLabel("WATCHLIST") === "Watchlist" && getPropGradeLabel("NO_PLAY") === "No Edge" && getPropGradeLabel("PENDING_DATA") === "Updating" && getPropGradeDescription("BEST_ANGLE").length > 0);
  ...
  check("member route uses product copy without operational internals", memberPropsSource.includes("Today’s board is updating.") && memberPropsSource.includes("latest complete market snapshot") && !["Supabase", "fixture", "flags", "Public picks hidden"].some((label) => memberPropsSource.includes(label)));
  ...
  const polishQa = existsSync(polishQaPath) ? JSON.parse(readFileSync(polishQaPath, "utf8")) as { featuredCardCount: { desktop: number; mobile: number }; defaultWorkspace: string; radarResearchQuestions: string[]; playerDirectoryVisible: boolean; focusedPlayerWorkspace: boolean; plainEnglishReader: boolean; continuousMarketBoard: boolean; neutralDefaultSort: string; modelSignalFilter: boolean; priceComparisonModes: string[]; advancedDiagnosticsHiddenFromBoard: boolean; projectionSideContradictionsCount: number; allModelStatesInspectable: boolean; searchVisible: boolean; commandCenterVisible: boolean; fixtureCoherence: boolean; publicDisplayEnabled: boolean; paperPersistenceEnabled: boolean; writesToSupabase: boolean } : null;
  check("player props polish QA report is complete", polishQa !== null && polishQa.featuredCardCount.desktop === 0 && polishQa.featuredCardCount.mobile === 0 && polishQa.defaultWorkspace === "Prop Researcher" && polishQa.radarResearchQuestions.length === 3 && polishQa.playerDirectoryVisible && polishQa.focusedPlayerWorkspace && polishQa.plainEnglishReader && polishQa.continuousMarketBoard && polishQa.neutralDefaultSort === "Player A-Z" && polishQa.modelSignalFilter && polishQa.priceComparisonModes.length === 2 && polishQa.advancedDiagnosticsHiddenFromBoard && polishQa.projectionSideContradictionsCount === 0 && polishQa.allModelStatesInspectable && polishQa.searchVisible && polishQa.commandCenterVisible && polishQa.fixtureCoherence && !polishQa.publicDisplayEnabled && !polishQa.paperPersistenceEnabled && !polishQa.writesToSupabase);
