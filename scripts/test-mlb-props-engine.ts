...
async function main() {
  ...
  check("actionable home-run markets lead with a plain binary model pick", propsUiSource.includes("function MilestoneProbabilityVisual") && propsUiSource.includes('return isHomeRunMarket(row) ? "1+ Home Run"') && propsUiSource.includes('if (isHomeRunMarket(row)) return isPositiveSignal(row) ? "Model pick" : "Model chance"') && propsUiSource.includes('return isPositiveSignal(row) ? "1+ Home Run" : pct(chance);') && propsUiSource.includes('if (isHomeRunMarket(row)) return null;'));
