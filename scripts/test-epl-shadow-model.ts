...
assert.match(sharpProvider, /private readonly league: string = SHARP_EPL_LEAGUE/);
assert.match(sharpProvider, /query: \{ sport: "soccer", league: this\.league, limit: 200 \}/);
...
assert.match(productionPipeline, /\.eq\("model_version", options\.modelRelease \?\? EPL_SHADOW_MODEL_RELEASE\)[\s\S]{0,100}\.is\("locked_at", null\)/, "T-60 discovery must default to current EPL release records while allowing an explicit competition release");
...
assert.match(dailyEdgeSports, /key: "ucl", label: "UCL", memberAvailable: false, inSeason: false/, "UCL availability must fail closed in the static registry");
assert.match(candidatePage, /resolveUclFeatureFlags\(\)\.member/, "the nested UCL member competition must be exposed only by the unified runtime gate");
