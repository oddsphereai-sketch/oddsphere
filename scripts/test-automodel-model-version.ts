 ...
 *   override > env > default champion "v2_2".
 *
...
async function main() {
  ...
  check("missing env → v2_2", resolveAutomodelVersion({}) === "v2_2");
  check("empty string env → v2_2", resolveAutomodelVersion({ [AUTOMODEL_VERSION_ENV]: "" }) === "v2_2");
  ...
  check("invalid value → v2_2 (with warn)", resolveAutomodelVersion({ [AUTOMODEL_VERSION_ENV]: "v3" }) === "v2_2");
  check("garbage value → v2_2", resolveAutomodelVersion({ [AUTOMODEL_VERSION_ENV]: "nonsense" }) === "v2_2");
  ...
  check("undefined override + env unset → v2_2", resolveEffectiveVersion(undefined, {}) === "v2_2");

  section("type narrowing");
  const v: AutomodelVersion = resolveAutomodelVersion({});
  check("return type is AutomodelVersion narrow", v === "v1" || v === "v2" || v === "v2_1" || v === "v2_2" || v === "shadow");
