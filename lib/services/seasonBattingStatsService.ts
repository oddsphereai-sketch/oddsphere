...
export const MLB_BATTING_STATS_FRESH_MS = 6 * 60 * 60 * 1000;
...
export function hasFreshTeamBattingCoverage(args: {
  ...
  const freshMs = args.freshMs ?? MLB_BATTING_STATS_FRESH_MS;
  ...
  for (const row of args.stats) {
    ...
    const usable =
      ...
      row.batting_pa >= 100;
    if (!usable || !Number.isFinite(updatedMs) || nowMs - updatedMs > freshMs) continue;
    counts.set(player.team_id, (counts.get(player.team_id) ?? 0) + 1);
  }
  return args.teamIds.every(
...
export async function refreshSlateSeasonBattingStats(args: {
  ...
  if (teamIds.length === 0) {
    return { status: "no_slate", teams_checked: 0, players_mapped: 0, provider_rows: 0, rows_written: 0, api_calls: 0 };
  }
