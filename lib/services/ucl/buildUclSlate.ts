...
let historicalPromise: Promise<Pick<Foundation, "historyMatches" | "trainingMatches" | "teamStats">> | null = null;
...
async function historical(provider: BallDontLieUclProvider) {
  ...
  if (stored?.schemaVersion === 2) {
    return { historyMatches: stored.historyMatches, trainingMatches: stored.trainingMatches, teamStats: stored.teamStats };
  }
  const seasons = await Promise.all(UCL_TRAINING_SEASONS.map((season) => provider.listMatches({ season })));
  const historyMatches = seasons.flat().filter((match) => match.status_state === "final");
  ...
  if (process.env.UCL_FOUNDATION_CACHE_WRITES_ENABLED === "true") {
    await writeUclHistoricalFoundation({ schemaVersion: 2, historyMatches, trainingMatches, teamStats }).catch(() => null);
  }
  return { historyMatches, trainingMatches, teamStats };
}
...
async function loadFoundation(provider: BallDontLieUclProvider): Promise<Foundation> {
  ...
  const promise = (async () => {
    historicalPromise ??= historical(provider).catch((error) => { historicalPromise = null; throw error; });
    const [prior, teams, seasonMatches] = await Promise.all([
      historicalPromise,
      provider.listTeams(UCL_CURRENT_SEASON),
      provider.listMatches({ season: UCL_CURRENT_SEASON }),
    ]);
...
export function selectUclMatchweek(groups: BdlUclMatch[][], now = new Date()): number {
  const future = groups.findIndex((group) => group.some((match) => Date.parse(match.date) >= now.getTime() - 6 * 60 * 60_000));
  return future >= 0 ? future + 1 : Math.max(1, groups.length);
}
...
export function buildUclSlate(requestedMatchweek?: number): Promise<UclSlate> {
  const cacheKey = requestedMatchweek ? `week:${requestedMatchweek}` : "default";
  const cached = slateCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.promise;
  const promise = buildUclSlateUncached(requestedMatchweek).catch((error) => { slateCache.delete(cacheKey); throw error; });
  slateCache.set(cacheKey, { expiresAt: Date.now() + SLATE_TTL_MS, promise });
  return promise;
}

async function buildUclSlateUncached(requestedMatchweek?: number): Promise<UclSlate> {
  ...
  const selected = requestedMatchweek && groups[requestedMatchweek - 1] ? requestedMatchweek : selectUclMatchweek(groups);
  const weekMatches = groups[selected - 1] ?? [];
  ...
  const matches = weekMatches.flatMap<EplShadowSlateMatch>((match) => {
    ...
    return [{
      ...
      prediction: fitAndPredictUcl({ training: foundation.trainingMatches, match, history: [...foundation.historyMatches, ...foundation.seasonMatches], context }),
