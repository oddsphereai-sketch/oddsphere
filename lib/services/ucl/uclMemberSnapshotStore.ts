...
export const UCL_MEMBER_SNAPSHOT_LIFECYCLE_RELEASE = "ucl_member_snapshot_lifecycle_2026_09_03_r1" as const;
...
export async function writeCurrentUclMemberSnapshot(input: { response: DailyEdgeResponse; matchweek: number; modelRelease: string; calibrationRelease: string }) {
  const lockSafe = preserveLockedEplGames(await readCurrentUclMemberSnapshot(), input.response);
