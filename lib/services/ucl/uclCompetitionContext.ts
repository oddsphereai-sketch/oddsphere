...
export const UCL_CONTEXT_RELEASE = "ucl_competition_context_2026_09_03_r1" as const;
export const UCL_SETTLEMENT_RELEASE = "ucl_regulation_settlement_2026_09_03_r1" as const;
...
function calendarStage(match: BdlUclMatch, reciprocal: boolean): UclStage {
  ...
  if (month >= 7 || month === 1) return match.season >= 2024 ? "league_phase" : "group_stage";
  if (!reciprocal) return month === 5 || month === 6 ? "final" : "knockout";
...
export function buildUclCompetitionContexts(matches: BdlUclMatch[]): Map<number, UclCompetitionContext> {
  ...
  for (const match of matches) {
    ...
    contexts.set(match.id, {
      ...
      regulationTime: true,
      advancementMarket: false,
      source: stage === "unknown" ? "unknown" : match.round_number === null ? "schedule_topology" : "provider_round_and_schedule",
    });
  }
  return contexts;
}
