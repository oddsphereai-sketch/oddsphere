...
import { UCL_COHERENT_OUTCOME_RELEASE, UCL_CALIBRATION_RELEASE } from "./uclModel";
...
function marketContext(market: MarketEdgeDto): MarketEdgeDto {
  return {
    ...
    soccerGradeContext: market.soccerGradeContext ? {
      ...
      calibration_label: market.soccerGradeContext.calibration_label.replace("EPL", "UCL"),
    } : market.soccerGradeContext,
  };
}
    ...
    games: base.games.map((game) => {
      ...
      return {
        ...
        soccerModelProvenance: game.soccerModelProvenance ? {
          ...
          coherentOutcomeRelease: UCL_COHERENT_OUTCOME_RELEASE,
          regulationTime: true,
        } : null,
        breakdown: {
          ...game.breakdown,
