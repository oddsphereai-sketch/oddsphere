...
import { resolveUclFeatureFlags, type UclFeatureEnvironment } from "./uclFeatureFlags";
...
export async function ingestUclFinalScores(input: {
  slateDate: string;
  apply: boolean;
  client?: typeof supabase;
  provider?: Pick<BallDontLieUclProvider, "listMatches">;
  featureEnv?: UclFeatureEnvironment;
}) {
  if (!resolveUclFeatureFlags(input.featureEnv).settlement) {
    return { updated: 0, apiEventsFetched: 0, finalizedCount: 0, heldSpecialFinals: 0, errors: [] as string[], disabled: true };
  }
  const client = input.client ?? supabase;
  let fixturesRaw: BdlUclMatch[];
  if (input.provider) {
    fixturesRaw = await input.provider.listMatches({ dates: [input.slateDate, addDay(input.slateDate)] });
  } else {
  ...
  fixturesRaw = await fixtureCache.promise;
  }
  const fixtures = fixturesRaw.filter((match) => etDate(match.date) === input.slateDate);
  ...
  for (const row of (data ?? []) as Array<{ id: number; external_id: number; status: string; home_score: number | null; away_score: number | null; inning_scores: unknown }>) {
    ...
    const { error: updateError } = await client.from("games").update(payload).eq("id", row.id).eq("sport", "soccer");
