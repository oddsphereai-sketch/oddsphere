  ...
  for (const date of opts.dates) {
    ...
    try {
      ...
      if (sport === "mlb") {
        ...
        if (process.env.UCL_PIPELINE_ENABLED === "true") {
          try {
      ...
      try {
        const gradeRes = await gradePredictionsForSlate({
          ...
          excludeCompetition: sport === "soccer"
            && process.env.UCL_PIPELINE_ENABLED === "true"
            && process.env.EPL_PIPELINE_ENABLED !== "true"
        ...
        if (sport === "soccer" && process.env.UCL_PIPELINE_ENABLED === "true") {
