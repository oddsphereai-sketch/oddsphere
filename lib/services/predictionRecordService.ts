...
export type CreateRecordsOptions = {
  ...
  /**
   * Dry-run-only writer output injection for exact release board audits.
   * It is rejected in apply mode and never used by scheduled writers.
   */
  auditPredictionsOverride?: ReadonlyArray<AutoModelOutput>;
};
...
export const TOTAL_VALIDATED_LEAN_FORWARD_STANDDOWN_RULE_ID =
  "total_validated_lean_forward_standdown_v1_2026_08_13";
...
export const TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_LEAN_RULE_ID =
  "total_mean_selector_original_under_lean_v1_2026_08_13";
export const TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MIN_ODDS = -145;
export const TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MAX_ODDS = 145;
...
export const ML_MARKET_LED_MOVEMENT_LEAN_RULE_ID =
  "ml_market_led_toward_move_playable_price_lean_v2_2026_08_12";
export const ML_MARKET_LED_MOVEMENT_MIN_OBSERVED_MODEL_PROB = 0.50;
export const ML_MARKET_LED_MOVEMENT_MIN_ODDS = -200;
...
export function resolveMlTightMarketPriceBestAngle(args: {
  ...
  const qualified =
    ...
    args.edgePct >= ML_TIGHT_MARKET_PRICE_MIN_EDGE_PCT &&
    args.edgePct < ML_TIGHT_MARKET_PRICE_MAX_EDGE_PCT_EXCLUSIVE &&
    args.oddsAmerican !== null &&
    Number.isFinite(args.oddsAmerican) &&
    args.oddsAmerican >= ML_TIGHT_MARKET_PRICE_MIN_ODDS &&
...
export function applyMlbNeutralConsensusGrades(records: PredictionRecordRow[]): PredictionRecordRow[] {
  return records.map((record) => {
    if (
      record.sport !== "mlb" ||
      record.market !== "moneyline" ||
      ...
      record.held ||
      record.odds_american === null ||
      record.odds_american < ML_NEUTRAL_CONSENSUS_MIN_ODDS ||
      record.odds_american > ML_NEUTRAL_CONSENSUS_MAX_ODDS ||
      record.data_quality_tier !== "high"
    ) return record;
    const snapshot = readRecordOrNull(record.snapshot_json) ?? {};
    const decision = readRecordOrNull(snapshot.decision_pipeline) ?? {};
...
): { betsPct: number | null; moneyPct: number | null; provider: string | null; pairComplete: boolean } {
  if (pickSide === null) return { betsPct: null, moneyPct: null, provider: null, pairComplete: false };
  const picked = rows.find((row) =>
    row.market_type === market &&
    sourceAwareSide(row) === pickSide &&
    (row.provider ?? "").toLowerCase() === provider
  );
  const oppositeSide = market === "moneyline"
    ? pickSide === "home" ? "away" : pickSide === "away" ? "home" : null
    : pickSide === "over" ? "under" : pickSide === "under" ? "over" : null;
  const opposite = oppositeSide === null ? undefined : rows.find((row) =>
    row.market_type === market &&
    sourceAwareSide(row) === oppositeSide &&
    (row.provider ?? "").toLowerCase() === provider
  );
  return {
    ...
    pairComplete: picked !== undefined && opposite !== undefined && sourceAwarePairScore(picked, opposite) <= 2,
  };
}
...
type MlbGradeLineDirection = ReturnType<typeof readLineDirection>;

function offeredPriceEdgePp(
  modelProbability: number | null,
  oddsAmerican: number | null,
): number | null {
  const breakEvenProbability = americanBreakEvenProbability(oddsAmerican);
  if (modelProbability === null || breakEvenProbability === null) return null;
  return (modelProbability - breakEvenProbability) * 100;
}
...
export function resolveMlbMoneylineConfidenceValueContextLean(args: {
  ...
  const lean =
    ...
    (args.side === "home" || args.side === "away") &&
    args.modelProbability !== null &&
    args.modelProbability >= MLB_ML_CONFIDENCE_VALUE_CONTEXT_MIN_MODEL_PROB &&
    priceEdgePp !== null &&
    priceEdgePp >= MLB_ML_CONFIDENCE_VALUE_CONTEXT_MIN_OFFERED_EDGE_PP &&
    args.sameSideProjectionGap !== null &&
    args.sameSideProjectionGap >= 0 &&
    directionalMarket &&
    !args.publicSplitConflict;
...
/**
 * Totals use the same confidence-first hierarchy with stricter market
 * treatment: adverse movement or a public/sharp conflict blocks the Lean.
 */
export function resolveMlbTotalConfidenceValueContextLean(args: {
  blocked: boolean;
  side: string | null;
  modelProbability: number | null;
  oddsAmerican: number | null;
  ...
  return {
    lean,
    offeredPriceEdgePp: priceEdgePp,
    reason: lean ? MLB_TOTAL_CONFIDENCE_VALUE_CONTEXT_LEAN_RULE_ID : null,
  };
}
...
export function resolveTotalMeanSelectorOriginalUnderLean(args: {
  blocked: boolean;
  correctionRejected: boolean;
  correctionRuleId: string | null;
  projectionConflict: boolean;
  side: string | null;
  oddsAmerican: number | null;
  pickedBetsPct: number | null;
  pickedMoneyPct: number | null;
  splitPairComplete: boolean;
  dataQualityTier: string | null;
}): { lean: boolean; reason: string | null } {
  const lean =
    !args.blocked &&
    args.correctionRejected &&
    args.correctionRuleId === TOTALS_MEAN_FLIP_RULE_ID &&
    args.projectionConflict &&
    args.side === "under" &&
    args.oddsAmerican !== null &&
    Number.isFinite(args.oddsAmerican) &&
    args.oddsAmerican >= TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MIN_ODDS &&
    args.oddsAmerican <= TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MAX_ODDS &&
    args.pickedBetsPct !== null &&
    args.pickedMoneyPct !== null &&
    args.splitPairComplete &&
    args.dataQualityTier === "high";
  return {
    lean,
    reason: lean ? TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_LEAN_RULE_ID : null,
  };
}
...
function resolveTotalSharpapiSupportLean(args: {
  blocked: boolean;
  side: string | null;
  oddsAmerican: number | null;
  lineDirection: "toward_pick" | "against_pick" | "neutral" | "unknown" | null;
  ...
  const lean =
    ...
    args.lineDirection !== "against_pick" &&
    moneyMinusBetsPct !== null &&
    moneyMinusBetsPct >= TOTAL_SHARPAPI_SUPPORT_MIN_MONEY_MINUS_BETS_PCT &&
    args.dataQualityTier === "high";
  return {
    lean,
    reason: lean ? TOTAL_SHARPAPI_SUPPORT_LEAN_RULE_ID : null,
    moneyMinusBetsPct,
  };
}
...
export function resolveMlbMarketAwareSideCorrection(
  ...
  // r54: Moneyline movement, split conflict, and regularization metadata are
  // contextual evidence about the independently modeled side. They may
  // reduce actionability, but they are not an opposite-side probability
  // model. The former path complemented the raw probability and then replaced
  // it with a cosmetic 55-60% display confidence, which broke side/probability
  // coherence. Keep the legacy selector only for the separately validated
  // Total split-conflict policy.
  if (args.market === "moneyline") {
    return { applied: false, reason: "moneyline_market_context_cannot_select_opposite_side" };
  }
...
export function resolveMlbMoneylineMarketContextSidePolicy(args: {
  side: string | null;
  lineDirection: "toward_pick" | "against_pick" | "neutral" | "unknown" | null;
  publicSplitConflict: boolean;
  distanceCapApplied: boolean;
}): {
  rule_id: typeof MLB_MONEYLINE_MARKET_CONTEXT_SIDE_POLICY_RULE_ID;
  originalSide: "home" | "away" | null;
  oppositeCandidateSide: "home" | "away" | null;
  action: "retain_original" | "stand_down_original";
  reasons: string[];
  oppositeSideRejected: true;
} {
  const originalSide = args.side === "home" || args.side === "away" ? args.side : null;
  const reasons = [
    args.lineDirection === "against_pick" ? "line_movement_against_pick" : null,
    args.publicSplitConflict ? "opposing_public_split_conflict" : null,
    args.distanceCapApplied ? "regularization_distance_cap_requires_independent_confirmation" : null,
  ].filter((reason): reason is string => reason !== null);
  return {
    rule_id: MLB_MONEYLINE_MARKET_CONTEXT_SIDE_POLICY_RULE_ID,
    originalSide,
    oppositeCandidateSide: oppositeMarketSide("moneyline", originalSide) as "home" | "away" | null,
    action: reasons.length > 0 ? "stand_down_original" : "retain_original",
    reasons,
    oppositeSideRejected: true,
  };
}
...
export function resolveMlbMarketAwareCorrectedPlayGrade(
  ...
  if (args.market === "moneyline") {
    return watchlist("corrected_ml_best_angle_stood_down_by_2026_07_20_recalibration");
  }
...
function buildMlRecord(
  ...
  // actionable; an intermediate inversion later reversed by calibration is
  // not a flip and receives no correction bet.
  const mlTrueInversionActionable =
    mlFlipped &&
    mlFinalSideChanged &&
    !mlPickCalibrated &&
    !mlMarketSideCorrected &&
  ...
  const mlTightMarketPriceBestAngle = resolveMlTightMarketPriceBestAngle({
    // The older clean-tight cohort has first priority. Both rules can point to
    // Best Angle, but a row receives exactly one promotion rule/audit stamp.
    blocked:
      mlNoBet ||
      mlFlipped ||
      mlPickCalibrated ||
  ...
  return {
    ...
    // intermediate inversion reversed downstream is not actionable.
    play_grade: mlTrueInversionActionable
      ? "lean"
      : mlMarketSideCorrected
    ...
    snapshot_json: {
      ...
      ml_confidence_value_context_lean: mlConfidenceValueContextLean.lean
        ? {
            ...
            odds_american: finalMlOdds,
            offered_price_edge_pp: mlConfidenceValueContextLean.offeredPriceEdgePp,
            minimum_offered_price_edge_pp:
              MLB_ML_CONFIDENCE_VALUE_CONTEXT_MIN_OFFERED_EDGE_PP,
            same_side_projection_gap: mlSameSideProjectionGap,
            line_direction: finalMlLineDirection,
            public_split_conflict: finalMlPublicSplitConflict,
            signed_market_resistance: mlSignedMarketResistance.standDown,
      ...
      ml_clean_tight_edge_best_angle_promotion: mlCleanTightEdgePromoted
        ...
        : null,
      ml_tight_market_price_best_angle_promotion: mlTightMarketPricePromoted
        ? {
            rule_id: ML_TIGHT_MARKET_PRICE_BEST_ANGLE_RULE_ID,
            action: "promote_to_best_angle",
            edge_pct: finalMlEdge,
            min_edge_pct: ML_TIGHT_MARKET_PRICE_MIN_EDGE_PCT,
            max_edge_pct_exclusive: ML_TIGHT_MARKET_PRICE_MAX_EDGE_PCT_EXCLUSIVE,
            odds_american: finalMlOdds,
            min_odds_inclusive: ML_TIGHT_MARKET_PRICE_MIN_ODDS,
      ...
      market_aware_side_correction: mlMarketSideCorrection.applied === true
        ? {
            ...
            validation_note:
              "MLB replay through 2026-07-19: market-aware Moneyline side corrections remain tracked, but v4 caps them at Market Aligned after the recent corrected Best Angle cohort went 4-4.",
          }
        : null,
      ml_market_context_side_policy: {
        rule_id: mlMarketContextSidePolicy.rule_id,
        original_side: mlMarketContextSidePolicy.originalSide,
        opposite_candidate_side: mlMarketContextSidePolicy.oppositeCandidateSide,
        opposite_side_rejected: mlMarketContextSidePolicy.oppositeSideRejected,
        candidate_action: mlMarketContextSidePolicy.action,
        effective_action:
          mlChampionGuardApplies && mlMarketContextSidePolicy.action === "stand_down_original"
            ? "stand_down_original"
            : "retain_original",
        reasons: mlMarketContextSidePolicy.reasons,
        policy_note:
          "Market context may grade or stand down the independently modeled Moneyline side; it cannot manufacture an opposite-side probability or action.",
      },
      ...
      ml_flip: mlFlipped
        ? {
            ...
            inversion_candidate_side: mlFlip.flippedSide,
            final_side: finalMlPick,
            final_side_changed: mlFinalSideChanged,
            reversed_by_pick_calibration: mlPickCalibrated && !mlFinalSideChanged,
...
function buildOuRecord(
  ...
  const v21 = (sp.v2_1_audit ?? {}) as Record<string, unknown>;
  const v22 = (sp.v2_2_audit ?? {}) as Record<string, unknown>;
  const totalRegimeCalibration = readRecordOrNull(v22.total_regime_calibration);
  const totalRegimeCalibrationApplied = readBoolish(totalRegimeCalibration?.applied);
  const legacyMarketSignalGradeInfluenceEnabled =
    readMarketIntelligenceV2Config().legacyMarketSignalGradeInfluenceEnabled;
  ...
  const ouMarketSideCorrection = !totalRegimeCalibrationApplied && !ouFlipped && !ouMarketFlipped && !ouDivergenceStandDown
    ? resolveMlbMarketAwareSideCorrection({
        market: "total",
        side: finalOuPick,
  ...
  const ouMidEdgeFlip = resolveTotalsMidEdgeFlip({
    currentSide: finalOuPick === "over" || finalOuPick === "under" ? finalOuPick : null,
    currentEdgePp: finalOuEdge,
    currentModelProb: finalOuModelProb,
    currentMarketProb: finalOuMarketProb,
    currentConfidence: finalOuConfidence,
    overOdds: oddsForGame?.ouOverOdds ?? null,
    underOdds: oddsForGame?.ouUnderOdds ?? null,
    priorCorrectionApplied:
  ...
  const ouMidEdgeFlipped = ouMidEdgeFlip.action === "flip";
  if (ouMidEdgeFlipped) {
    finalOuPick = ouMidEdgeFlip.flippedSide;
    finalOuOdds = ouMidEdgeFlip.flippedOdds;
    ouCorrectionRuleId = TOTALS_MID_EDGE_FLIP_RULE_ID;
    ouCorrectionKind = "mid_edge_inversion";
    ouCorrectedSideModelProb = ouMidEdgeFlip.flippedSideModelProb;
    ouCorrectedSideMarketProb = ouMidEdgeFlip.flippedMarketProb;
    ouCorrectedSideEdgePp = ouMidEdgeFlip.flippedEdgePp;
    finalOuConfidence = ouMidEdgeFlip.recommendationConfidence;
    finalOuModelProb = ouMidEdgeFlip.recommendationConfidence / 100;
    finalOuMarketProb = ouMidEdgeFlip.flippedMarketProb;
    finalOuEdge = ouMidEdgeFlip.flippedEdgePp;
    finalOuBetLine = pricedTotalLineForSide(finalOuPick) ?? finalOuBetLine;
    finalOuLineMovement = buildLineMovementSnapshot(
      openersForGame, currentLinesForGame, signalsForGame, "total", finalOuPick,
      freshnessReferenceMs,
  ...
  const ouRejectedCandidate = ouCorrectionRejected
    ...
    : null;
  if (ouCorrectionRejected) {
    finalOuPick = pred.predicted_ou_side;
    finalOuOdds = ouOddsAmerican;
    finalOuBetLine = ouBetLine;
    finalOuConfidence = pred.ou_confidence;
    finalOuModelProb = ouModelProb;
    finalOuMarketProb = ouMarketProb;
    finalOuEdge = ouEdgePp;
  ...
  const ouRawProjectionChampionApplied =
    !totalRegimeCalibrationApplied && ouRawProjectionChampion.applied === true;
  if (ouRawProjectionChampionApplied && ouRawProjectionChampion.applied) {
  ...
  const ouMeanSelectorOriginalUnderLean = resolveTotalMeanSelectorOriginalUnderLean({
    blocked:
      ouMissingActionableMarket ||
      ouDivergenceStandDown ||
      isExplicitNoBetReason(explicitOuNoBetReason),
    correctionRejected: ouCorrectionRejected,
    correctionRuleId: ouCorrectionRuleId,
    projectionConflict: ouProjectionConflict,
    side: finalOuPick,
  ...
  const ouConfidenceValueContextLean = resolveMlbTotalConfidenceValueContextLean({
    blocked:
      ...
      ouRawProjectionChampionApplied ||
      readBoolish(sp.v2_provisional) ||
      readStringOrNull(readRecordOrNull(sp.mlb_data_completeness)?.status) ===
        "incomplete_missing_required_data",
    side: finalOuPick,
    modelProbability: ouChampionModelProb,
    oddsAmerican: finalOuOdds,
  ...
  const ouLegacyValidatedLean = resolveTotalValidatedLean({
  ...
  const ouValidatedLeanForwardStandDown = ouLegacyValidatedLean.lean && !ouCorrectionRejected;
  ...
  const ouSharpapiSupportLean = resolveTotalSharpapiSupportLean({
    blocked:
      trackedOuFinalBestAngle ||
      ouNoBet ||
      ouMeanSelectorOriginalUnderLean.lean ||
  ...
  const ouModelLeanRetained =
    !trackedOuFinalBestAngle &&
    !ouNoBet &&
    !ouValidatedLeanForwardStandDown &&
    finalOuPublicPlayGrade === "lean" &&
    finalOuModelProb !== null &&
    finalOuOdds !== null &&
  ...
  const ouUnvalidatedLeanCap =
    ...
    !ouConfidenceValueContextLean.lean &&
    !ouModelLeanRetained;
  ...
  return {
    ...
    snapshot_json: {
      ...
      decision_pipeline: {
        ...
        grade_source:
          trackedOuFinalBestAngle && ouModelBestAngleRetained
            ...
            : ouModelLeanRetained
              ? "calibrated_model"
              : ouPromotedBestAngle || ouMeanSelectorOriginalUnderLean.lean || ouUnderLowTicketResistanceLean.lean || ouSharpapiSupportLean.lean || ouConfidenceValueContextLean.lean
                ? "additive_rule"
                : null,
        paired_policy: {
          demotion: TOTAL_VALIDATED_LEAN_FORWARD_STANDDOWN_RULE_ID,
          promotions: [
            TOTAL_CLEAN_CONFIRMED_BEST_ANGLE_RULE_ID,
            TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_LEAN_RULE_ID,
            TOTAL_UNDER_LOW_TICKET_RESISTANCE_LEAN_RULE_ID,
        ...
        final_side_changed: didFinalSideChange(pred.predicted_ou_side, finalOuPick),
      },
      ...
      total_clean_confirmed_best_angle_promotion: ouPromotedBestAngle
        ...
        : null,
      total_confidence_value_context_lean: ouConfidenceValueContextLean.lean
        ? {
            rule_id: MLB_TOTAL_CONFIDENCE_VALUE_CONTEXT_LEAN_RULE_ID,
            action: "promote_or_retain_as_lean",
            model_probability: finalOuModelProb,
      ...
      total_lean_recalibration_cap: ouUnvalidatedLeanCap
        ...
        : null,
      total_validated_lean_forward_standdown: ouValidatedLeanForwardStandDown
        ? {
            rule_id: TOTAL_VALIDATED_LEAN_FORWARD_STANDDOWN_RULE_ID,
            superseded_rule_id: TOTAL_VALIDATED_LEAN_RULE_ID,
            action: "stand_down_from_lean",
            strength: ouLegacyValidatedLean.strength,
            model_prob: finalOuModelProb,
            ...
            validation_note:
              "Forward release-head audit after launch went 5-8, -23.5% ROI. The historical threshold fit no longer earns an actionable grade and is replaced by a chronologically stable original-Under selector.",
          }
        : null,
      total_mean_selector_original_under_lean: ouMeanSelectorOriginalUnderLean.lean
        ? {
            rule_id: TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_LEAN_RULE_ID,
            action: "promote_original_under_to_lean",
            rejected_correction_rule_id: ouCorrectionRuleId,
            projection_conflict: ouProjectionConflict,
            odds_american: finalOuOdds,
            min_odds: TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MIN_ODDS,
            max_odds: TOTAL_MEAN_SELECTOR_ORIGINAL_UNDER_MAX_ODDS,
            ...
            data_quality_tier: readStringOrNull(sp.v2_data_quality_tier),
            validation_note:
              "Forward frozen-context replay of rejected mean-side correction candidates: original Unders went 19-6-1, +41.1% ROI, positive in all four chronological weeks, with date-block bootstrap P(profitable)=0.9952. Over candidates were rejected at 1-7.",
          }
        : null,
      total_under_low_ticket_resistance_lean: ouUnderLowTicketResistanceLean.lean
        ...
        : null,
      total_sharpapi_support_lean: ouSharpapiSupportLean.lean
        ? {
            rule_id: TOTAL_SHARPAPI_SUPPORT_LEAN_RULE_ID,
            action: "promote_to_lean",
            odds_american: finalOuOdds,
            min_odds: TOTAL_SHARPAPI_SUPPORT_MIN_ODDS,
            max_odds: TOTAL_SHARPAPI_SUPPORT_MAX_ODDS,
            line_direction: ouLineDirection,
            picked_bets_pct: ouValidatedSharpPublicSplit.betsPct,
            picked_money_pct: ouValidatedSharpPublicSplit.moneyPct,
            split_provider: ouValidatedSharpPublicSplit.provider,
            money_minus_bets_pct: ouSharpapiSupportLean.moneyMinusBetsPct,
            min_money_minus_bets_pct: TOTAL_SHARPAPI_SUPPORT_MIN_MONEY_MINUS_BETS_PCT,
            data_quality_tier: readStringOrNull(sp.v2_data_quality_tier),
            validation_note:
              "Historically nonactionable high-quality MLB Unders with exact selected-side SharpAPI money at least 10 points above tickets, no movement against the pick, and a -145 through +145 price went 17-5 and remained positive in train, validation, and holdout. The Over branch was rejected after flat validation and holdout.",
          }
        : null,
      ...
      totals_correction_rejection: ouCorrectionRejected
        ? {
            ...
            rejected_candidate_odds: ouRejectedCandidate?.odds ?? null,
            rejected_candidate_line: ouRejectedCandidate?.line ?? null,
            rejected_candidate_model_prob: ouRejectedCandidate?.modelProb ?? null,
            rejected_candidate_market_prob: ouRejectedCandidate?.marketProb ?? null,
            rejected_candidate_edge_pp: ouRejectedCandidate?.edgePp ?? null,
            market_aware_reasons:
              ouMarketSideCorrection.applied === true ? ouMarketSideCorrection.reasons : null,
          }
        : null,
      /* Removed as an official action in decision release r1. */
      ou_flip: null,
    },
  };
}
...
export async function createPredictionRecords(
  ...
  if (apply && opts.auditPredictionsOverride !== undefined) {
    throw new Error(
      "predictionRecordService.createPredictionRecords: auditPredictionsOverride " +
        "is dry-run only and cannot be combined with apply=true.",
    );
  }
  ...
  if (opts.auditPredictionsOverride !== undefined) {
    const auditPredictionByExternalId = new Map(
      opts.auditPredictionsOverride.map((prediction) => [
        prediction.game_external_id,
        prediction,
      ]),
    );
    for (const game of games) {
      const prediction = auditPredictionByExternalId.get(game.external_id);
      if (prediction === undefined) continue;
      const persisted = predictionByGameId.get(game.id);
      predictionByGameId.set(game.id, {
        id: persisted?.id ?? -game.id,
        game_id: game.id,
        predicted_ml_winner: prediction.predicted_ml_winner,
        ml_confidence: prediction.ml_confidence,
        predicted_ou_side: prediction.predicted_ou_side,
        ou_confidence: prediction.ou_confidence,
        predicted_nrfi: prediction.predicted_nrfi,
        nrfi_confidence: prediction.nrfi_confidence,
        prediction_source: prediction.prediction_source,
        is_override: false,
        locked_at: persisted?.locked_at ?? null,
        computed_at: persisted?.computed_at ?? new Date().toISOString(),
        sport_specific: prediction.sport_specific,
        predicted_home_score: prediction.predicted_home_score,
        predicted_away_score: prediction.predicted_away_score,
        ml_grade: persisted?.ml_grade ?? null,
        ou_grade: persisted?.ou_grade ?? null,
        nrfi_grade: persisted?.nrfi_grade ?? null,
        ml_signal_type: persisted?.ml_signal_type ?? null,
        ou_signal_type: persisted?.ou_signal_type ?? null,
        nrfi_signal_type: persisted?.nrfi_signal_type ?? null,
        ml_market_signal: persisted?.ml_market_signal ?? null,
        ou_market_signal: persisted?.ou_market_signal ?? null,
        nrfi_market_signal: persisted?.nrfi_market_signal ?? null,
      });
    }
  }
