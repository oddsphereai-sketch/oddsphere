...
function buildFiRecord(
  ...
  const noBetValue = isTossUp || fiWriterNoBet;
  ...
  // FI tracking must copy the FI v2 writer grade, not recreate the old card
  // ladder from confidence. The Daily Edge route reads the same
  // sport_specific.fi_v2_audit.fi_play_grade field pre-lock, and locked rows
  // resolve from prediction_records.play_grade + best_angle. Keeping these in
  // lockstep is what makes "grade what users saw" true for Best Angle, Lean,
  // Toss-Up, held, and no-bet FI states.
  const FI_LEAN_CONFIDENCE_FLOOR_PCT = 58;
  const legacyFiPlayGrade: string | null =
    !isTossUp &&
    typeof pred.nrfi_confidence === "number" &&
  ...
  const fiPlayGrade: string | null =
    fiWriterPlayGrade ??
    (isTossUp ? "toss_up" : legacyFiPlayGrade);
