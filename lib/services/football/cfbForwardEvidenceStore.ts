...
export async function readCfbForwardWriterEvidence(args: {
  ...
  for (const row of metadataRows) {
    ...
    const recoveryRelease = currentRelease || row.evidence_release === CFB_FORWARD_PRICE_PREVIOUS_EVIDENCE_SCHEMA_RELEASE;
    if (currentRelease) {
  ...
  for (let index = 0; index < ids.length; index += CFB_FORWARD_WRITER_PAYLOAD_BATCH_SIZE) {
    const { data, error } = await args.client
      ...
      .in("evidence_release", [CFB_FORWARD_EVIDENCE_SCHEMA_RELEASE, CFB_FORWARD_PRICE_PREVIOUS_EVIDENCE_SCHEMA_RELEASE])
