...
function normalizeStoredRow(row: StoredRow): CfbForwardStoredEvidence {
  ...
  if (payload.schemaRelease !== CFB_FORWARD_EVIDENCE_SCHEMA_RELEASE || payload.game.providerGameId !== row.provider_game_id || payload.stage !== row.stage || payload.capturedAt !== new Date(row.captured_at).toISOString()) {
