...
export async function loadLockedPredictionEvidence(args: {
  ...
  const { data, error } = await supabase
    ...
    .select("id,game_prediction_id,game_id,external_id,sport,slate_date,game_date,matchup,market,pick,side,line_value,odds_american,model_probability,market_probability,edge,play_grade,no_bet,no_bet_reason,confidence,locked_at,snapshot_json,game_predictions(locked_at)")
    .eq("sport", args.sport)
